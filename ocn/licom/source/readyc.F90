...
!
!#ifdef SPMD
      call exch_boundary(riu,kmm1)
!#endif
!#ifdef DEBUG
!      call chk_var1(riu,'iu',0)
!      if (mytid.eq.0) print*,"test"
!#endif
!
!         DO J = 1,jmt
!         DO I = 2,IMM
...
!
         DO K = 1,ITNU(i,j)-1
               wp1(K)= VIT (I,J,K+1)* (AT (I,J,K,1)+(AT (I,J,K+1,1)-AT (I,J,K,1))* &
                               DZP (K)/(DZP(K)+DZP(K+1)))
               wp2(K)= VIT (I,J,K+1)* (AT (I,J,K,2)+(AT (I,J,K+1,2)-AT (I,J,K,2))* &
                               DZP (K)/(DZP(K)+DZP(K+1)))
...
!lhl0608                               DZP (K)/(DZP(K)+DZP(K+1)))
               wp3(K)= VIT (I,J,K+1)* (pdensity (I,J,K)+(pdensity (I,J,K+1)-pdensity(I,J,K))* &
