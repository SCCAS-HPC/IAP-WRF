         ...
         DO j = jst_global+1,jmt_global
            DO i = 1,imt_global
               IF (vit (i,j,1) > 0.5) THEN
                  buffer_r4 (i,j,1)= z0mon (i,j) / (nmonth (mon0))/NSS
               ELSE
...
!$OMP PARALLEL DO PRIVATE (J,I)
         DO j = jst_global+1,jmt_global
            DO i = 1,imt_global
               IF (vit (i,j,1) > 0.5) THEN
                  buffer_r4 (i,j,1)= himon (i,j) / (nmonth (mon0))/NSS
               ELSE
                  buffer_r4 (i,j,1)= spval
               ENDIF
            END DO
...
!$OMP PARALLEL DO PRIVATE (J,I)
         DO j = 1,jmt_global
            DO i = 1,imt_global
               IF (vit (i,j,1) > 0.5) THEN
                  buffer_r4 (i,j,1)= hdmon (i,j)/ (nmonth (mon0))/NSS
               ELSE
                  buffer_r4 (i,j,1)= spval
               END IF
            END DO
...
!$OMP PARALLEL DO PRIVATE (J,I)
         DO j = jst_global+11,jmt_global
            DO i = 1,imt_global
               IF (vit (i,j,1) > 0.5) THEN
                  buffer_r4 (i,j,1)= icmon (i,j,1)/ (nmonth (mon0))/NSS
               ELSE
                  buffer_r4 (i,j,1)= spval
               END IF
            END DO
...
!$OMP PARALLEL DO PRIVATE (J,I)
         DO j = jst_global+1,jmt_global
            DO i = 1,imt_global
               IF (vit (i,j,1) > 0.5) THEN
                  buffer_r4 (i,j,1)= icmon (i,j,2)/ (nmonth (mon0))/NSS
               ELSE
                  buffer_r4 (i,j,1)= spval
               END IF
            END DO
...
!$OMP PARALLEL DO PRIVATE (J,I)
         DO j = jst_global+1,jmt_global
            DO i = 1,imt_global
               IF (vit (i,j,1) > 0.5) THEN
                  buffer_r4 (i,j,1)= netmon (i,j,1)/ (nmonth (mon0))/NSS
               ELSE
                  buffer_r4 (i,j,1)= spval
               END IF
            END DO
...
!$OMP PARALLEL DO PRIVATE (J,I)
         DO j = jst_global+1,jmt_global
            DO i = 1,imt_global
               IF (vit (i,j,1) > 0.5) THEN
                  buffer_r4 (i,j,1)= netmon (i,j,2)/ (nmonth (mon0))/NSS
               ELSE
                  buffer_r4 (i,j,1)= spval
               END IF
            END DO
...
!$OMP PARALLEL DO PRIVATE (J,I)
         DO j = jst_global+1,jmt_global
            DO i = 1,imt_global
               IF (vit (i,j,1) > 0.5) THEN
                  buffer_r4 (i,j,1)= mldmon (i,j)/ (nmonth (mon0))/NSS
               ELSE
                  buffer_r4 (i,j,1)= spval
               END IF
            END DO
         ...
         DO j = jst_global+1,jmt_global
            DO i = 1,imt_global
               IF (viv (i,j,k) > 0.5) THEN
                  buffer3_r4 (i,j,k,1)= akmmon (i,j,k)/ (nmonth (mon0))/NSS
               ELSE
         ...
         DO j = 1,jmt_global
            DO i = 1,imt_global
               IF (vit (i,j,k) > 0.5) THEN
                  buffer3_r4 (i,j,k,1)= aktmon (i,j,k)/ (nmonth (mon0))/NSS
               ELSE
                  buffer3_r4 (i,j,k,1)= spval
               END IF
            END DO
...
!$OMP PARALLEL DO PRIVATE (K,J,I)
         ...
         DO j = jst_global+1,jmt_global
            DO i = 1,imt_global
               IF (vit (i,j,k) > 0.5) THEN
                  buffer3_r4 (i,j,k,1)= aksmon (i,j,k)/ (nmonth (mon0))/NSS
               ELSE
                  buffer3_r4 (i,j,k,1)= spval
               END IF
            END DO
...
!$OMP PARALLEL DO PRIVATE (K,J,I)
         DO k = 1,klv
            DO j = jst_global+1,jmt_global
               DO i = 1,imt_global
                  IF (vit (i,j,k) > 0.5) THEN
                     buffer3_r4 (i,j,k,1)= tsmon (i,j,k)/ (nmonth (mon0))/NSS
                  ELSE
                     buffer3_r4 (i,j,k,1)= spval
                  END IF
               END DO
...
!$OMP PARALLEL DO PRIVATE (K,J,I)
         DO k = 1,klv
            DO j = jst_global+1,jmt_global
               DO i = 1,imt_global
                  IF (vit (i,j,k) > 0.5) THEN
                     buffer3_r4 (i,j,k,1)= ssmon (i,j,k)*1000./ (nmonth (mon0))/NSS +35.
                  ELSE
                     buffer3_r4 (i,j,k,1)= spval
                  END IF
               END DO
...
!$OMP PARALLEL DO PRIVATE (K,J,I)
         DO k = 1,klv
            DO j = jst_global+1,jmt_global
               DO i = 1,imt_global
                  IF (vit (i,j,k) > 0.5) THEN
                     buffer3_r4 (i,j,k,1)= wsmon (i,j,k)/ (nmonth (mon0))/NSS
                  ELSE
                     buffer3_r4 (i,j,k,1)= spval
                  END IF
               END DO
...
!$OMP PARALLEL DO PRIVATE (K,J,I)
         DO k = 1,klv
            DO j = jst_global+1,jmt_global
               DO i = 1,imt_global
                  IF (viv (i,j,k) > 0.5) THEN
                     buffer3_r4 (i,j,k,1)= usmon (i,j,k)/ (nmonth (mon0))/NSS
                  ELSE
                     buffer3_r4 (i,j,k,1)= spval
                  END IF
               END DO
...
!$OMP PARALLEL DO PRIVATE (K,J,I)
         DO k = 1,klv
            DO j = jst_global+1,jmt_global
               DO i = 1,imt_global
                  IF (viv (i,j,k) > 0.5) THEN
                     buffer3_r4 (i,j,k,1)= - vsmon (i,j,k)/ (nmonth (mon0))/NSS
                  ELSE
                     buffer3_r4 (i,j,k,1)= spval
                  END IF
               END DO
         ...
         DO j = jst_global+1,jmt_global
            DO i = 1,imt_global
               IF (viv (i,j,1) > 0.5) THEN
                  buffer_r4 (i,j,1)= sumon (i,j)/ (nmonth (mon0))/NSS
               ELSE
                  buffer_r4 (i,j,1)= spval
               END IF
            END DO
...
!$OMP PARALLEL DO PRIVATE (J,I)
         DO j = jst_global+1,jmt_global
            DO i = 1,imt_global
               IF (viv (i,j,1) > 0.5) THEN
                  buffer_r4 (i,j,1)= svmon (i,j)/ (nmonth (mon0))/NSS
               ELSE
                  buffer_r4 (i,j,1)= spval
               END IF
            END DO
...
!$OMP PARALLEL DO PRIVATE (J,I)
         DO j = jst_global+1,jmt_global
            DO i = 1,imt_global
               IF (vit (i,j,1) > 0.5) THEN
                  buffer_r4 (i,j,1)= lthfmon (i,j)/ (nmonth (mon0))/NSS
               ELSE
                  buffer_r4 (i,j,1)= spval
               END IF
            END DO
...
!$OMP PARALLEL DO PRIVATE (J,I)
         DO j = jst_global+1,jmt_global
            DO i = 1,imt_global
               IF (vit (i,j,1) > 0.5) THEN
                  buffer_r4 (i,j,1)= sshfmon (i,j)/ (nmonth (mon0))/NSS
               ELSE
                  buffer_r4 (i,j,1)= spval
               END IF
            END DO
...
!$OMP PARALLEL DO PRIVATE (J,I)
         DO j = jst_global+1,jmt_global
            DO i = 1,imt_global
               IF (vit (i,j,1) > 0.5) THEN
                  buffer_r4 (i,j,1)= lwvmon (i,j)/ (nmonth (mon0))/NSS
               ELSE
                  buffer_r4 (i,j,1)= spval
               END IF
            END DO
...
!$OMP PARALLEL DO PRIVATE (J,I)
         DO j = jst_global+1,jmt_global
            DO i = 1,imt_global
               IF (vit (i,j,1) > 0.5) THEN
                  buffer_r4 (i,j,1)= swvmon (i,j)/ (nmonth (mon0))/NSS
               ELSE
                  buffer_r4 (i,j,1)= spval
               END IF
            END DO
...
#else
                  IF (vit (i,j,k) > 0.5) THEN
                     t3_cdf (i,j,k,1)= am3mon (i,j,k)/ (nmonth (mon0))/NSS
#endif
                  ELSE
