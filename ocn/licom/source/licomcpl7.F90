...
!
!   !INTERFACE:
  !subroutine licom_init_mct(EClock, cdata_o, x2o_o, o2x_o,r2x_o, NLFilename)
  subroutine licom_init_mct(EClock, cdata_o, x2o_o, o2x_o, NLFilename) !LPF 20121219
!
!   !DESCRIPTION:
...
!
!   !INTERFACE:
!  subroutine licom_run_mct( EClock, cdata_o, x2o_o, o2x_o,r2x_o)
...
!----------------------------------------------------------------------
    LOGMSG()
    !call licom_import_mct(x2o_o,r2x_o)
...
!     COMPENSATE THE LOSS OF GROSS MASS
    ...
    LOGMSG()
         CALL ADDPS
!      if (mytid == 0) write(*,*)ii,jj,NCC,"OK------run12"
...
!
    ...
    LOGMSG()
         call flux_cpl
!      if (mytid == 0) write(*,*)ii,jj,NCC,"OK------run13"
#endif
...
!----------------------------------------------------------------------
    LOGMSG()
    call licom_export_mct(o2x_o)
!      if (mytid == 0) write(*,*)ii,jj,NCC,"OK------run16"
...
!         lihuimin 2012.7.16
!
  subroutine licom_import_mct(x2o_o) !LPF 20121219
    type(mct_aVect)   , intent(inout) ::  x2o_o
    ! local
