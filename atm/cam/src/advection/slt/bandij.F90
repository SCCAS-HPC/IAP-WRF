...
!------------------------------Arguments--------------------------------
  ...
  real(r8), intent(in)  :: lamp(plon,beglev:endlev)    ! longitude coordinates of dep. points
  real(r8), intent(in)  :: phip(plon,beglev:endlev)    ! latitude  coordinates of dep. points
  integer , intent(in)  :: nlon               ! number of longitudes
  integer , intent(out) :: iband(plon,beglev:endlev,4) ! longitude index of dep. points
  integer , intent(out) :: jband(plon,beglev:endlev)   ! latitude  index of dep. points
!-----------------------------------------------------------------------
!
!---------------------------Local workspace-----------------------------
!
...
!$OMP PARALLEL DO PRIVATE (K, I)
  do k=beglev,endlev
!!     print*, 'k =', k
!!     print*, 'phip(1,k) =', phip(1,k), ' phip(19,k) =', phip(19,k)
     do i = 1,nlon
!
! Latitude indices.
!
