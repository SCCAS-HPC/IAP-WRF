...
! 
! Modify: Zhang He, 2008.4.17, change vertical coordinate from eta to sigma
!         Jiang Jinrong and Zhang He, 2013-02-28, for 2D parallel
!-----------------------------------------------------------------------
!
! $Id: xqmass.F90,v 1.1.2.6 2004/09/23 17:20:10 erik Exp $
! $Author: erik $
!
!-----------------------------------------------------------------------
...
!---------------------------Arguments-----------------------------------
  ...
  real(r8), intent(in)  :: qo(plon,beglev:endlev)  ! q old            (pre -SLT)
  real(r8), intent(in)  :: qn(plon,beglev:endlev)  ! q new            (post-SLT)
  real(r8), intent(in)  :: xo(plon,beglev:endlev,pcnst)  ! old constituents (pre -SLT)
  real(r8), intent(in)  :: xn(plon,beglev:endlev,pcnst)  ! new constituents (post-SLT)
...
!!                                                ! based sigma part of hybrid grid
  real(r8), intent(inout) :: hwxl(pcnst,4)     ! partial integrals 
...
!!           hwbk(4) = hwbk(4) + xdxqdq*pdelb(i,k)
           ...
           hwk(1) = hwk(1) + xq1   *pdel(i,k)
           hwk(2) = hwk(2) + xqdq  *pdel(i,k)
           hwk(3) = hwk(3) + xdxq1 *pdel(i,k)
           hwk(4) = hwk(4) + xdxqdq*pdel(i,k)
        end do
...
!!        hwblat(4) = hwblat(4) + hwbk(4)*etamsq(k)
        ...
        hwlat(1) = hwlat(1) + hwk(1)
        hwlat(2) = hwlat(2) + hwk(2)*etamid(k)
        hwlat(3) = hwlat(3) + hwk(3)*etamid(k)
        hwlat(4) = hwlat(4) + hwk(4)*etamsq(k)
     end do
!
