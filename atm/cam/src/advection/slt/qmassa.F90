...
! 
! Author: J. Olson
! Modified: Jiang Jinrong and Zhang He, 2013-02-28, for 2D parallel
!         : Zhang He, 2013-03-19, pdeld(:,:) --> pdeld(plon,beglev:endlev)
! 
!-----------------------------------------------------------------------
!
! $Id$
...
!------------------------------Arguments--------------------------------
  ...
  real(r8), intent(in)  :: q3(plon,beglev:endlev,pcnst)  ! constituents
  real(r8), intent(in)  :: q0(plon,beglev:endlev,pcnst)  ! constituents at begining of time step
  real(r8), intent(in)  :: pdel(plon,beglev:endlev)      ! pressure diff between interfaces
  real(r8), intent(out) :: hw1lat(pcnst)        ! accumulator
  real(r8), intent(in),optional  :: pdeld(plon,beglev:endlev)  ! dry pressure difference for dry-type constituents
...
!
     elseif (cnst_get_type_byind(m).eq.'dry' ) then  ! dry type constituents
        ...
        if (  dycore_is ('EUL').or.dycore_is ('IAP') ) then           ! EUL dycore computes pdeld in time filter
