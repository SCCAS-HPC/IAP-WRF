...
use cam_history
! juanxiong he
use nudging_driver,     only: fdda, analysis_interval, guv, gt, gq, &
                           if_zfac_uv, if_zfac_t, if_zfac_q, &
                           k_zfac_uv, k_zfac_t, k_zfac_q,&
                           k_zbot_uv, k_zbot_t, k_zbot_q,&
                           vert_int,data_src, fdda_sfc, data_sfcsrc
use cam_control_mod
...
!            it is not supported.  
  ...
  ! nudging, juanxiong he
  namelist /cam_inparm/ fdda, analysis_interval, guv, gt, gq, &
                        if_zfac_uv, if_zfac_t, if_zfac_q, &
                        k_zfac_uv, k_zfac_t, k_zfac_q,&
                        k_zbot_uv, k_zbot_t, k_zbot_q,&
                        vert_int,data_src, fdda_sfc, data_sfcsrc
...
! 
   ...
   ! juanxiong he
   call mpibcast (fdda  ,1,mpiint,0,mpicom)
   call mpibcast (fdda_sfc  ,1,mpiint,0,mpicom)
   call mpibcast (analysis_interval  ,1,mpiint,0,mpicom)
   call mpibcast (guv  ,1,mpir4,0,mpicom)
   call mpibcast (gt  ,1,mpir4,0,mpicom)
   call mpibcast (gq  ,1,mpir4,0,mpicom)
   call mpibcast (if_zfac_uv  ,1,mpiint,0,mpicom)
   call mpibcast (if_zfac_t  ,1,mpiint,0,mpicom)
   call mpibcast (if_zfac_q  ,1,mpiint,0,mpicom)
   call mpibcast (k_zfac_uv  ,1,mpiint,0,mpicom)
   call mpibcast (k_zfac_t  ,1,mpiint,0,mpicom)
   call mpibcast (k_zfac_q  ,1,mpiint,0,mpicom)
   call mpibcast (k_zbot_uv  ,1,mpiint,0,mpicom)
   call mpibcast (k_zbot_t  ,1,mpiint,0,mpicom)
   call mpibcast (k_zbot_q  ,1,mpiint,0,mpicom)
   call mpibcast (vert_int  ,1,mpiint,0,mpicom)
   call mpibcast (data_src  ,1,mpiint,0,mpicom)
   call mpibcast (data_sfcsrc  ,1,mpiint,0,mpicom)
