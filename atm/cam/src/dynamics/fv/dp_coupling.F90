...
! !PUBLIC MEMBER FUNCTIONS:
      PUBLIC d_p_coupling, p_d_coupling, wrf_to_dynamics
...
!
! !INTERFACE: 
  subroutine d_p_coupling(grid, phys_state, phys_tend, pbuf, dyn_out)
...
!-----------------------------------------------------------------------
  ...
  subroutine wrf_to_dynamics(grid, phys_state, phys_tend, &
                          dyn_in, dtime, zvir, cappa, ptop)
...
! !DESCRIPTION:
!
!   Coupler for converting wrf output variables into dynamics input
!   variables
!-----------------------------------------------------------------------
! LOCAL VARIABLES:
...
! Local workspace
    ...
    integer :: tsize                  ! amount of data per grid point passed to physics
    integer, allocatable, dimension(:,:) :: bpter
                                     ! offsets into block buffer for unpacking
                                     ! data
    integer :: cpter(pcols,0:pver)   ! offsets into chunk buffer for packing data
    integer :: iqa, iqb, iqc, iqd, mq     ! used for tracer transpose grouping
...
!$omp parallel do private (lchnk, ncol, i, k, m, cpter)
          do lchnk = begchunk,endchunk
             ...
             do i=1,ncol
                cbuffer(cpter(i,0):cpter(i,0)+3+1) = 0.0_r8
             end do
