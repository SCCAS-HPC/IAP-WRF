...
!------------------------------Arguments--------------------------------
    ...
    real(r8), intent(inout) :: t3  (plon, plev, beglat:endlat)  ! temperature
    real(r8), intent(inout) :: u3  (plon, plev, beglat:endlat)  ! u-wind component
    real(r8), intent(inout) :: v3  (plon, plev, beglat:endlat)  ! v-wind component
    real(r8), intent(inout) :: q3  (plon, plev, pcnst, beglat:endlat) ! constituents
!
!---------------------------Local workspace-----------------------------
#if (! defined SPMD)
    real(r8) :: buf1(1), buf2(1)     ! transpose buffers
...
!$OMP PARALLEL DO PRIVATE (LCHNK, NCOL, I, K, M, LONS, LATS)
       ...
       do lchnk = begchunk,endchunk
          ...
          do k=1,plev
             do i=1,ncol
               if(phys_state(lchnk)%t(i,k).ne.0) then
                t3(lons(i),k,lats(i))   = phys_state(lchnk)%t (i,k)
                u3(lons(i),k,lats(i))   = phys_state(lchnk)%u (i,k)
                v3(lons(i),k,lats(i))   = phys_state(lchnk)%v (i,k)
          ...
          do i=1,ncol
            if(phys_state(lchnk)%ps(i).ne.0)   ps(lons(i),lats(i))   = phys_state(lchnk)%ps(i)
          end do
...
!$OMP PARALLEL DO PRIVATE (LCHNK, NCOL, CPTER, I, K, M)
       do lchnk = begchunk,endchunk
          ...
          do i=1,ncol
             buf2(cpter(i,0)) = phys_state(lchnk)%ps(i)
          end do
...
!$OMP PARALLEL DO PRIVATE (J, BPTER, I, K, M)
       ...
       do j=beglat,endlat
          ...
          do i=1,nlon(j)
            if( buf1(bpter(i,0)) .ne. 0.0 )   ps(i,j) = buf1(bpter(i,0))
          end do
...
!$OMP PARALLEL DO PRIVATE (K, I, M)
          do k=1,plev
             ...
             do i=1,nlon(j)
               ...
               if( buf1(bpter(i,k)).ne.0) then
                 t3(i,k,j) = buf1(bpter(i,k))
                 u3(i,k,j) = buf1(bpter(i,k)+1)
                 v3(i,k,j) = buf1(bpter(i,k)+2)
                 q3(i,k,1,j) = buf1(bpter(i,k)+3)
               endif
...
!===============================================================================
  ...
  subroutine physics_to_uv(phys_state, ps, u3, v3)  ! juanxiong
!------------------------------------------------------------------------------
! Coupler for converting wrf input variables into dynamics input variables
!------------------------------Arguments--------------------------------
    use constituents, only: cnst_get_type_byind
...
!$OMP PARALLEL DO PRIVATE (LCHNK, NCOL, I, K, M, LONS, LATS)
       ...
       if (tsize*max(block_buf_nrecs,chunk_buf_nrecs) > spmdbuf_siz) then
          call endrun ('nudging_to_dynamics: communication buffers (spmdbuf_siz) too small')
       endif
       ...
       do lchnk = begchunk,endchunk
          ...
          do k=1,plev
             ...
             do i=1,ncol
                ...
                buf2(cpter(i,k))   = phys_state(lchnk)%u (i,k)
                buf2(cpter(i,k)+1) = phys_state(lchnk)%v (i,k)
...
!$OMP PARALLEL DO PRIVATE (J, BPTER, I, K, M)
 ...
 ! ps doen't take part i the nudging
 !         do i=1,nlon(j)
 !           if( buf1(bpter(i,0)) .ne. 0.0 )   ps(i,j) = buf1(bpter(i,0))
 !         end do
...
!$OMP PARALLEL DO PRIVATE (K, I, M)
          do k=1,plev
             ...
             do i=1,nlon(j)
                 ...
                 u3(i,k,j) = buf1(bpter(i,k))
                 v3(i,k,j) = buf1(bpter(i,k)+1)
    ...
    return
  end subroutine physics_to_uv

!===============================================================================
  subroutine uv_to_physics(ps, u3, v3, phys_state)
!------------------------------------------------------------------------------
! Coupler for converting dynamics output variables into physics input variables
! also writes dynamics variables (on physics grid) to history file
!------------------------------------------------------------------------------
...
!$OMP PARALLEL DO PRIVATE (LCHNK, NCOL, I, K, M, LONS, LATS)
       ...
       if (tsize*max(block_buf_nrecs,chunk_buf_nrecs) > spmdbuf_siz) then
          call endrun ('dynamics_to_coupling: communication buffers (spmdbuf_siz) too small')
       endif
...
! ps doesn't take part in the nudging
...
!$OMP PARALLEL DO PRIVATE (K, I, M)
          do k=1,plev
             do i=1,nlon(j)
                ...
                buf1(bpter(i,k))   = u3  (i,k,j)
                buf1(bpter(i,k)+1) = v3  (i,k,j)
...
!          end do
          ...
          do k=1,plev
             do i=1,ncol
                ...
                phys_state(lchnk)%u    (i,k)   = buf2(cpter(i,k))
                phys_state(lchnk)%v    (i,k)   = buf2(cpter(i,k)+1)
    ...
    return
  end subroutine uv_to_physics

!===============================================================================

  subroutine physics_to_tq(phys_state,ps,t3,q3)
!------------------------------------------------------------------------------
! Coupler for converting wrf input variables into dynamics input variables
!------------------------------Arguments--------------------------------
    use constituents, only: cnst_get_type_byind
    ...
    type(physics_state),intent(in), dimension(begchunk:endchunk) :: phys_state
    real(r8), intent(inout) :: ps  (plon, beglat:endlat)            ! surface pressure
    real(r8), intent(inout) :: t3  (plon, plev, beglat:endlat)  ! u-wind component
    real(r8), intent(inout) :: q3  (plon, plev, 1, beglat:endlat)  ! v-wind component
!
!---------------------------Local workspace-----------------------------
#if (! defined SPMD)
    real(r8) :: buf1(1), buf2(1)     ! transpose buffers
       ...
       do lchnk = begchunk,endchunk
          ...
          do k=1,plev
             ...
             do i=1,ncol
                ...
                buf2(cpter(i,k))   = phys_state(lchnk)%t (i,k)
                buf2(cpter(i,k)+1) = phys_state(lchnk)%q (i,k,1)
...
!$OMP PARALLEL DO PRIVATE (K, I, M)
    ...
    return
  end subroutine physics_to_tq

!===============================================================================

  subroutine tq_to_physics(ps,t3,q3,phys_state)
!------------------------------------------------------------------------------
! Coupler for converting dynamics output variables into physics input variables
! also writes dynamics variables (on physics grid) to history file
!------------------------------------------------------------------------------
...
!------------------------------Arguments--------------------------------
    real(r8), intent(in) :: ps  (plon, beglat:endlat)            ! surface pressure
    real(r8), intent(in) :: t3  (plon, plev, beglat:endlat)  ! temperature
    real(r8), intent(in) :: q3  (plon, plev, 1, beglat:endlat)  ! u-wind component
    type(physics_state), intent(inout), dimension(begchunk:endchunk) :: phys_state
!
!---------------------------Local workspace-----------------------------
#if (! defined SPMD)
...
!          end do
          ...
          do k=1,plev
             do i=1,ncol
                ...
                phys_state(lchnk)%t    (i,k)   = buf2(cpter(i,k))
                phys_state(lchnk)%q    (i,k,1)   = buf2(cpter(i,k)+1)
