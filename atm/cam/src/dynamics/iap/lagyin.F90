...
!-----------------------------------------------------------------------
   ...
   use pmgrid,       only: plon, plev, beglev, endlev, npr_z
...
!
   ...
   real(r8), intent(in) :: ydp(plon,beglev:endlev)           ! y-coordinates of departure pts.
!
   integer, intent(in) :: jdp(plon,beglev:endlev)            ! j-index of departure point coord.
   integer, intent(in) :: jcen                               ! current latitude
   integer, intent(in) :: nlon
!
! Output arguments
!
   real(r8), intent(out) :: fdp(plon,beglev:endlev,pf)       ! interpolants at the horiz. depart. pt.
!
!-----------------------------------------------------------------------
!
...
!
   ...
   real(r8) term1(plon,beglev:endlev)          ! |
   real(r8) term2(plon,beglev:endlev)          ! |
   real(r8) term3(plon,beglev:endlev)          ! |
   real(r8) term4(plon,beglev:endlev)          ! |
!
   integer jdpval,icount,ii,indx(plon),nval
   integer k,icount1,ierr
!
!-----------------------------------------------------------------------
!
   if( ppdy .ne. 4) then
...
!$OMP PARALLEL DO PRIVATE (K, INDX, II, I, YMY3, YMY4, COEF12, YMY2, YMY1, COEF34)
         do k=beglev,endlev
            call whenieq(nlon,jdp(1,k),1,jdpval,indx,nval)   !zhh
            icount1 = icount1 + nval
!
            do ii = 1,nval
               i=indx(ii)
...
!jjr
        if(npr_z.gt.1) then
#if (defined SPMD)
           call mpi_allreduce(icount1,icount,1,mpiint,mpi_sum,comm_z,ierr)
#endif
        else
           icount=icount1
        endif
