module iap_fft
!------------------------------------------------------------------------------------------------
! Purpose: An efficient highly composite FFT( Fast Fourier Transformation) algorithm
! Original version : ECWMF
! Reference : 
! Reconstructed to module : ZhangHe
! Completed : 2006.01.12
!------------------------------------------------------------------------------------------------
   use shr_kind_mod, only: r8 => shr_kind_r8
   use IAP_grid, only: IM

!   implicit none
   save
   public

   integer,  parameter :: N0 = IM
   real(r8) :: TRIGS0(3*N0/2+1)
   integer  :: IFAX0(13)
...
!================================================================================================   
   SUBROUTINE IAP_FFT99(A,WORK,TRIGS,IFAX,INC,JUMP,N,LOT,ISIGN)
!!   SUBROUTINE IAP_FFT99(A,WORK,INC,JUMP,LOT,ISIGN)
!
...
!              IAP_FFT99 WRITTEN FOR XMP/YMPs BY DR. CLIVE TEMPERTON OF
!              ECMWF.
!
!              THE PACKAGE IAP_FFT99F CONTAINS SEVERAL USER-LEVEL ROUTINES:
!
!            SUBROUTINE IAP_SET99
...
!
!            SUBROUTINES IAP_FFT99 AND IAP_FFT991
...
!                  +          WORK(M*(N+1))
!
!                   CALL IAP_SET99 (TRIGS, IFAX, N)
!                   CALL IAP_FFT99 (A,WORK,TRIGS,IFAX,INC,JUMP,N,M,ISIGN)
!
!              SEE THE INDIVIDUAL WRITE-UPS FOR IAP_SET99, IAP_FFT99, AND
!              IAP_FFT991 BELOW, FOR A DETAILED DESCRIPTION OF THE
!              ARGUMENTS.
!
...
!-----------------------------------------------------------------------
!
! SUBROUTINE IAP_SET99 (TRIGS, IFAX, N)
!
! PURPOSE      A SET-UP ROUTINE FOR IAP_FFT99 AND IAP_FFT991.  IT NEED ONLY BE
...
!               THE DOCUMENTATION FOR IAP_FFT99 AND IAP_FFT991 FOR THE
...
!               IN IFAX(2),IFAX(3),...  IF IAP_SET99 IS CALLED WITH N ODD,
...
!-----------------------------------------------------------------------
!
! SUBROUTINE IAP_FFT991 (A,WORK,TRIGS,IFAX,INC,JUMP,N,M,ISIGN)
!                       AND
! SUBROUTINE IAP_FFT99 (A,WORK,TRIGS,IFAX,INC,JUMP,N,M,ISIGN)
!
! PURPOSE      PERFORM A NUMBER OF SIMULTANEOUS REAL/HALF-COMPLEX
!              PERIODIC FOURIER TRANSFORMS OR CORRESPONDING INVERSE
!              TRANSFORMS, USING ORDINARY SPATIAL ORDER OF GRIDPOINT
!              VALUES (IAP_FFT991) OR EXPLICIT CYCLIC CONTINUITY IN THE
!              GRIDPOINT VALUES (IAP_FFT99).  GIVEN A SET
...
!              OPTIMIZED ROUTINE IAP_FFT991 WRITTEN FOR XMP/YMPs BY
...
!
!              TRIGS
!               AN ARRAY SET UP BY IAP_SET99, WHICH MUST BE CALLED FIRST.
!
!              IFAX
!               AN ARRAY SET UP BY IAP_SET99, WHICH MUST BE CALLED FIRST.
!
...
!
!               FOR IAP_FFT991, X(0), X(1), X(2),...,X(N-1),0,0.
!               FOR IAP_FFT99, X(N-1),X(0),X(1),X(2),...,X(N-1),X(0).
...
!               AND IAP_FFT99 SHOULD BE USED (BECAUSE OF THE EXPLICIT CYCLIC
!               CONTINUITY).
!
...
!
!-----------------------------------------------------------------------
!zf      DIMENSION A(N),WORK(N),TRIGS(N),IFAX(1)
      real(r8) :: A((N+2)*LOT)
      real(r8) :: WORK((N+1)*LOT)
      real(r8) :: TRIGS(3*N/2+1)
      integer  ::  IFAX(13)
!
!     SUBROUTINE "IAP_FFT99" - MULTIPLE FAST REAL PERIODIC TRANSFORM
...
!
   30 CONTINUE
      CALL IAP_FFT99A(A,WORK,TRIGS,INC,JUMP,N,LOT)
      IGO=60
!
!     COMPLEX TRANSFORM
!     -----------------
!
   ...
   50 CONTINUE
!      print*, 'Abefore =', A      
	  CALL IAP_VPASSM(A(IA),A(IA+INC),WORK(1),WORK(2),TRIGS,INK,2,JUMP,NX,LOT,NH,IFAX(K+1),LA)  
...
!      print*, 'INC =', INC
!=======================================
	  CALL IAP_VPASSM(WORK(1),WORK(2),A(IA),A(IA+INC),TRIGS,2,INK,NX,JUMP,LOT,NH,IFAX(K+1),LA)   
...
!
  130 CONTINUE
      CALL IAP_FFT99B(WORK,A,TRIGS,INC,JUMP,N,LOT)
!
  140 CONTINUE
      RETURN 
   END SUBROUTINE
...
!===================================================================================
   SUBROUTINE IAP_FFT99A(A,WORK,TRIGS,INC,JUMP,N,LOT)
!zhh      SUBROUTINE IAP_FFT99A(A,WORK,INC,JUMP,LOT)         !zhh
!-----------------------------------------------------------------------------------
!     SUBROUTINE IAP_FFT99A - PREPROCESSING STEP FOR IAP_FFT99, ISIGN=+1
!     (SPECTRAL TO GRIDPOINT TRANSFORM)
!-----------------------------------------------------------------------------------
!zf      DIMENSION A(N),WORK(N),TRIGS(N)
      real(r8) :: A((N+2)*LOT)
      real(r8) :: WORK((N+1)*LOT)
      real(r8) :: TRIGS(3*N/2+1)
!-----------------------------------------------------------------------------------
...
!===================================================================================
   SUBROUTINE IAP_FFT99B(WORK,A,TRIGS,INC,JUMP,N,LOT)
!zhh      SUBROUTINE IAP_FFT99B(WORK,A,INC,JUMP,LOT)          !zhh
!-----------------------------------------------------------------------------------
!     SUBROUTINE IAP_FFT99B - POSTPROCESSING STEP FOR IAP_FFT99, ISIGN=-1
!     (GRIDPOINT TO SPECTRAL TRANSFORM)
!-----------------------------------------------------------------------------------
!zf      DIMENSION WORK(N),A(N),TRIGS(N)
      real(r8) :: A((N+2)*LOT)
      real(r8) :: WORK((N+1)*LOT)
      real(r8) :: TRIGS(3*N/2+1)
!-----------------------------------------------------------------------------------
...
!===================================================================================
   SUBROUTINE IAP_FFT991(A,WORK,TRIGS,IFAX,INC,JUMP,N,LOT,ISIGN)
!zhh      SUBROUTINE IAP_FFT991(A,WORK,INC,JUMP,LOT,ISIGN)                   !zhh
!-----------------------------------------------------------------------------------
!     SUBROUTINE "IAP_FFT991" - MULTIPLE REAL/HALF-COMPLEX PERIODIC
!     FAST FOURIER TRANSFORM
!
!     SAME AS IAP_FFT99 EXCEPT THAT ORDERING OF DATA CORRESPONDS TO
!     THAT IN MRFFT2
!
!     PROCEDURE USED TO CONVERT TO HALF-LENGTH COMPLEX TRANSFORM
!     IS GIVEN BY COOLEY, LEWIS AND WELCH (J. SOUND VIB., VOL. 12
...
!===================================================================================
!zhh      SUBROUTINE IAP_SET99 (TRIGS, IFAX, N)
   SUBROUTINE IAP_SET99                      !zhh
!zf      DIMENSION IFAX(13),TRIGS(1)
!zhh      REAL      TRIGS(3*N/2+1)
!zhh      integer ::   IFAX(13)
!
...
!-----------------------------------------------------------------------------------
      DATA MODE /3/
!---------------------------------------------------------------
      CALL IAP_FAX (IFAX0, N0, MODE)                  !zhh
!---------------------------------------------------------------
      I = IFAX0(1)
      IF (IFAX0(I+1) .GT. 5 .OR. N0 .LE. 4) IFAX0(1) = -99
      IF (IFAX0(1) .LE. 0 ) THEN 
        WRITE(6,*) ' IAP_SET99 -- INVALID N'
        STOP 'IAP_SET99'
      ENDIF
!---------------------------------------------------------------
      CALL IAP_FFTRIG (TRIGS0, N0, MODE)              !zhh
!---------------------------------------------------------------
      RETURN
   END SUBROUTINE

!===================================================================================
   SUBROUTINE IAP_FAX(IFAX,N,MODE)
!!      SUBROUTINE IAP_FAX(MODE)
!zf      DIMENSION IFAX(10)
      integer :: IFAX(13)                !zhh
!----------------------------------------------------------------------------------
      NN=N
...
!===================================================================================
   SUBROUTINE IAP_FFTRIG(TRIGS,N,MODE)
!zhh      SUBROUTINE IAP_FFTRIG(MODE)
!zf      DIMENSION TRIGS(1)
      real(r8) :: TRIGS(3*N/2+1)            !zhh
!----------------------------------------------------------------------------------
      PI=2.0*ASIN(1.0)
      IMODE=IABS(MODE)
      NN=N
...
!===================================================================================
   SUBROUTINE IAP_VPASSM(A,B,C,D,TRIGS,INC1,INC2,INC3,INC4,LOT,N,IFAC,LA)
!-----------------------------------------------------------------------------------
!     SUBROUTINE "IAP_VPASSM" - MULTIPLE VERSION OF "VPASSA"
