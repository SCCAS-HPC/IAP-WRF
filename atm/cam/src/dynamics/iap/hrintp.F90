...
!
   real(r8), intent(in) :: fb (plond,beglev:endlev,pkcnst,beglatex:endlatex) ! input fields
   real(r8), intent(in) :: fxl(plond,beglev:endlev,pf    ,beglatex:endlatex) ! left  x-derivs
   real(r8), intent(in) :: fxr(plond,beglev:endlev,pf    ,beglatex:endlatex) ! right x-derivs
   ...
   real(r8), intent(in) :: xdp(plon,beglev:endlev)          ! x-coord of dep. pt.
   real(r8), intent(in) :: ydp(plon,beglev:endlev)          ! y-coord of dep. pt.
!
   integer, intent(in) :: idp(plon,beglev:endlev,4)     ! i index of dep. pt.
   integer, intent(in) :: jdp(plon,beglev:endlev)       ! j index of dep. pt.
...
!
   ...
   real(r8), intent(out) :: fyb (plon,beglev:endlev,pf)      ! y-derivatives at bot of int.
   real(r8), intent(out) :: fyt (plon,beglev:endlev,pf)      ! y-derivatives at top of int.
   real(r8), intent(out) :: fdp (plon,beglev:endlev,pf)      ! horizontal interpolants
