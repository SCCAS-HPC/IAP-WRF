...
!
   real(r8), intent(in) :: fint(plon,beglev:endlev,ppdy,pf)   ! x-interpolants
   real(r8), intent(in) :: fyb (plon,beglev:endlev,pf)        ! y-derivatives at bottom of interval
   real(r8), intent(in) :: fyt (plon,beglev:endlev,pf)        ! y-derivatives at top    of interval
   real(r8), intent(in) :: y   (platd)               ! latitude grid coordinates
   real(r8), intent(in) :: dy  (platd)               ! intervals between latitude grid pts.
   real(r8), intent(in) :: ydp (plon, beglev:endlev)           ! lat. coord of departure point.
!
   integer, intent(in) :: jdp (plon,beglev:endlev)        ! lat. index of departure point.
   integer, intent(in) :: nlon
!
! Output arguments
!
   real(r8), intent(out) :: fdp (plon,beglev:endlev,pf)        ! y-interpolants
...
!
   ...
   integer m                 ! index
!
   real(r8) dyj(plon,beglev:endlev)   ! latitude interval containing dep. pt.
   real(r8) yb (plon,beglev:endlev)   ! |
   real(r8) yt (plon,beglev:endlev)   ! |
   real(r8) hb (plon,beglev:endlev)   ! | -- interpolation coefficients
   real(r8) ht (plon,beglev:endlev)   ! |
   real(r8) dhb(plon,beglev:endlev)   ! |
   real(r8) dht(plon,beglev:endlev)   ! |
!
!-----------------------------------------------------------------------
!
   jb = ppdy/2
