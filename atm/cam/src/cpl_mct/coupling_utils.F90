...
!  use module_domain
  ...
  use pmgrid, only: beglonxy, endlonxy, beglatxy, endlatxy, plev,plat,plon        !by Wang Yuzhu
  use physics_types, only: physics_state, physics_tend  ! by Wang Yuzhu
  use stepon, only: phys_state

implicit none
   type metgrid_info_atm
      ...
      real*4, pointer,  dimension(:,:):: xlat,xlon
      real*4, pointer,  dimension(:,:):: tsk,sst,snow,xice,pslv,psfc,&
                                      xland,ht,sstold,xiceold
      real*4, pointer,  dimension(:,:,:):: u3d,v3d,w3d,q3d,z3d,t3d,p3d,rh3d
      real*4, pointer,  dimension(:,:,:):: soilt,soilm,soildepth,soilthick     !jjr  
   end type metgrid_info_atm
...
!--------------------------------------------------------------------------
  ...
  public :: set_metgrid
  public :: initial_metgrid
  public :: wrf_import_cam_test
  public :: metgrid_info_atm
!  public :: pack_mgrid
!--------------------------------------------------------------------------
! Private interfaces
!--------------------------------------------------------------------------
  ...
  private :: spline
  private :: splint
  private :: interpolate_to_standard_levels
  private :: metgrid_oncetogather
  private :: metgrid_alltogather2
  private :: gather_whole_field_r
  private :: gather_whole_field_r2
...
!      use module_configure
      implicit none
      type(metgrid_info_atm) :: mgrid
      integer            :: num_soil_layers
	  integer            :: num_cam_levs
      mgrid%num_metgrid_soil_levels = 4 !jjr
...
!      mgrid%jde = model_config_rec%cam_ny
      mgrid%jde = plat
      mgrid%iproj =  0
!      print *,'ide',plat,'jde',plon,'metgrid_levevs',num_cam_levs,'soil_layers',num_soil_layers
    end subroutine set_metgrid
	

!===============================================================================
    subroutine initial_metgrid(mgrid, ids, ide, jds, jde, &
                               num_metgrid_levels, num_soil_layers )
      use spmd_dyn,only:iam
	  implicit none
      integer,intent(in)::ids, ide, jds, jde, num_metgrid_levels, num_soil_layers 
      type(metgrid_info_atm)::mgrid
      ...
      if(.not.associated(mgrid%soilt))           allocate( mgrid%soilt(ids:ide, 1:num_soil_layers, jds:jde)  )
      if(.not.associated(mgrid%soilm))          allocate( mgrid%soilm(ids:ide, 1:num_soil_layers, jds:jde)  )
      if(.not.associated(mgrid%soilthick))     allocate( mgrid%soilthick(ids:ide, 1:num_soil_layers, jds:jde)  )
      if(.not.associated(mgrid%soildepth))     allocate( mgrid%soildepth(ids:ide, 1:num_soil_layers, jds:jde)  )
...
!      if(iam==0)print*,'mgridx is initialized'
      mgrid%u3d = 0
      mgrid%v3d = 0
      mgrid%w3d = 0
      mgrid%q3d = 0
...
!      if(iam==0) write(20000,*)'first_sst',first_sst,'loop',loop
...
!			print*,'test hhq 3',i,j
	  ...
	  do k = kstart, grid%num_metgrid_levels ! num_metgrid_levels=num_cam_levels+1
            ...
            p3d(lons(i)-ips+1,k-1,lats(i)-jps+1) = 100000.0_8*a(grid%num_metgrid_levels-k+1) &
			                                            + cam_state(j)%ps(i)*b(grid%num_metgrid_levels-k+1)
	        rh3d(lons(i)-ips+1,k-1,lats(i)-jps+1) = cam_state(j)%rh(i,grid%num_metgrid_levels-k+1) 
			                                        ! relative humidity, use w3d 
!            grid%q3d(i,k,j)       = 0  
...
!           grid%xland(i,j)= cam_in(j)%landfrac(i) ! used as land
           grid%snow(lons(i),lats(i)) = (grid%xland(lons(i),lats(i)) *cam_in(j)%snowhland(i) &
		                                     +grid%xice(lons(i),lats(i)) * cam_in(j)%snowhice(i))
!           grid%snow(i,j) = (grid%xland(i,j) *cam_in(j)%snowhland(i)  +grid%xice(i,j) * cam_in(j)%snowhice(i))
           ...
           do k=1, grid%num_metgrid_soil_levels
            grid%soildepth(lons(i),k,lats(i))       =  cam_in(j)%soildepth(i,k)*100.0  
			                                           ! The unit is supposed as cm
!            grid%soildepth(i,k,j)       =  cam_in(j)%soildepth(i,k)*100.0  ! The unit is supposed as cm
            grid%soilthick(lons(i),k,lats(i))       = cam_in(j)%soilthick(i,k)*100.0   
			                                           !The unit is supposed as cm
...
!         if(iam==0.and.first_sst) write(200,*) 'atm_lon',grid%xlon,'atm_xlat',grid%xlat,'atm_q3d',grid%q3d, &
!		                                       'atm_pslv',grid%pslv,'atm_ht',grid%ht, &
!		                                       'atm_sst',grid%sst,'atm_xice',grid%xice,'atm_sstold',grid%sstold, &
!											   'atm_xiceold',grid%xiceold,'atm_tsk',grid%tsk, &
!											   'atm_xland',grid%xland,'atm_snow',grid%snow, &
!											   'atm_soildepth',grid%soildepth,'atm_soilthick',grid%soilthick, &
!											   'atm_soilt',grid%soilt,'atm_soilm',grid%soilm, &
!											   'atm_u3d',grid%u3d,'atm_v3d',grid%v3d,'atm_t3d',grid%t3d, &
!											   'atm_rh3d',grid%rh3d,'atm_z3d',grid%z3d,'atm_p3d',grid%p3d
!         if(iam==31.and.first_sst) write(201,*)'atm_lon',grid%xlon,'atm_xlat',grid%xlat,'atm_q3d',grid%q3d, &
!                                                      'atm_pslv',grid%pslv,'atm_ht',grid%ht,&
!                                                       'atm_sst',grid%sst,'atm_xice',grid%xice,'atm_sstold',grid%sstold,&
!                                                                                           'atm_xiceold',grid%xiceold,'atm_tsk',grid%tsk,&
!                                                                                           'atm_xland',grid%xland,'atm_snow',grid%snow,&
