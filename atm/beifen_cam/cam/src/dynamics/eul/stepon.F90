...
subroutine stepon_run1( ztodt, phys_state, phys_tend, wrf_state, wrf_tend,&
                        dyn_in, dyn_out, nudging_state, nudging_tend, &
                        fdda, twoway_coupling, twoway_nudging) ! juanxiong he
!----------------------------------------------------------------------- 
! 
...
!----------------------------------------------------------------------- 
  ...
  type(physics_tend),  intent(out)   :: wrf_tend(begchunk:endchunk)  ! juanxiong he
  ...
  type(physics_tend),  intent(out)   :: nudging_tend(begchunk:endchunk)
  ...
  if(twoway_coupling) then
    call wrf_to_dynamics(wrf_state, ps(1,beglat,n3m1), t3(1,1,beglat,n3m1), u3(1,1,beglat,n3m1), &
                         v3(1,1,beglat,n3m1), q3(1,1,1,beglat,n3m1))
  endif
...
!----------------------------------------------------------------------- 
  ...
  type(physics_tend), intent(inout):: pyhs_tend(begchunk:endchunk) ! juanxiong he
  ...
  type(cam_out_t), intent(inout) :: cam_out(begchunk:endchunk)  ! juanxiong he
  logical, intent(in) :: twoway_coupling ! juanxiong he
  ...
  if(twoway_coupling) then
  call d_p_coupling (ps(1,beglat,n3m1), t3(1,1,beglat,n3m1), u3(1,1,beglat,n3m1), &
                     v3(1,1,beglat,n3m1), q3(1,1,1,beglat,n3m1), &
                     omga, phis, cam_state, cam_tend, pbuf, pdeld(:,:,:,n3m1)) ! added by Juanxiong He
  end if
