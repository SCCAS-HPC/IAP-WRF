...
!
! !INTERFACE:
subroutine stepon_run1( dtime_out, phys_state, phys_tend, wrf_state, wrf_tend, &
...
!-----------------------------------------------------------------------
! !USES:
   use dp_coupling,       only: d_p_coupling,p_d_coupling, wrf_to_dynamics
   ...
   use cam_history,   only: outfld ! juanxiong he
!-----------------------------------------------------------------------
! !OUTPUT PARAMETERS:
!
   ...
   type (T_FVDYCORE_GRID), pointer :: grid  ! juanxiong he
   ...
   type(physics_state), intent(inout) :: nudging_state(begchunk:endchunk)
   type(physics_tend),  intent(inout)  :: nudging_tend(begchunk:endchunk)
   type(physics_state), intent(inout) :: cam_state_ac(begchunk:endchunk)
   type(physics_tend),  intent(inout)  :: cam_tend_ac(begchunk:endchunk)
   logical, intent(inout) :: twoway_coupling
   integer, intent(in) :: twoway_nudging
   integer, intent(in) :: fdda
   integer  :: i, j, k
   integer  :: im, jm, km, ifirstxy, ilastxy, jfirstxy, jlastxy, idim
   real(r8),dimension(:,:),allocatable :: u3s_tmp,v3s_tmp,t3s_tmp,q3s_tmp
   real(r8),dimension(:,:,:),allocatable :: u3s_old,v3s_old,t3s_old,q3s_old
...
!BOC
   ...
   ! nudging
   if(fdda.gt.0) then   
    call d_p_coupling(dyn_state%grid, nudging_state, nudging_tend, pbuf, dyn_out)
   endif

   ! twoway coupling
   if(twoway_coupling) then    
    grid => get_dyn_state_grid()
    call wrf_to_dynamics(grid, wrf_state, wrf_tend, &
                         dyn_in, dtime, zvir, cappa, ptop)
   endif

   ! dynamical tendency
   im     =  dyn_state%grid%im
   jm     =  dyn_state%grid%jm
   km     = dyn_state%grid%km
   ifirstxy =  dyn_state%grid%ifirstxy
   ilastxy  =  dyn_state%grid%ilastxy
   jfirstxy =  dyn_state%grid%jfirstxy
   jlastxy  =  dyn_state%grid%jlastxy
   allocate(u3s_tmp(ifirstxy:ilastxy,km))
   allocate(v3s_tmp(ifirstxy:ilastxy,km))
   allocate(t3s_tmp(ifirstxy:ilastxy,km))
   allocate(q3s_tmp(ifirstxy:ilastxy,km))
   allocate(u3s_old(ifirstxy:ilastxy,jfirstxy:jlastxy,km))
   allocate(v3s_old(ifirstxy:ilastxy,jfirstxy:jlastxy,km))
   allocate(t3s_old(ifirstxy:ilastxy,jfirstxy:jlastxy,km))
   allocate(q3s_old(ifirstxy:ilastxy,jfirstxy:jlastxy,km))
   do j = jfirstxy, jlastxy
    do k = 1, km
     do i = ifirstxy, ilastxy
        u3s_old(i,j,k)=dyn_out%u3s (i,j,k)
        v3s_old(i,j,k)=dyn_out%v3s (i,j,k)
        t3s_old(i,j,k)=dyn_out%pt(i,j,k)
        q3s_old(i,j,k)=dyn_out%tracer(i,j,k,1)
     enddo
    enddo
   end do 
   ...
   do j = jfirstxy, jlastxy
      do k = 1, km
         do i = ifirstxy, ilastxy
            u3s_tmp (i,k) = u3s_old (i,j,k)-dyn_out%u3s (i,j,k)
            v3s_tmp (i,k) = v3s_old(i,j,k)-dyn_out%v3s (i,j,k)
            t3s_tmp (i,k) = t3s_old(i,j,k)-dyn_out%pt(i,j,k)
            q3s_tmp (i,k) = q3s_old(i,j,k)-dyn_out%tracer(i,j,k,1)
         enddo
      enddo
      call outfld ('UTEND      ', u3s_tmp   , idim, j )
      call outfld ('VTEND      ', v3s_tmp   , idim, j )
      call outfld ('TTEND      ', t3s_tmp   , idim, j )
      call outfld ('QTEND      ', q3s_tmp   , idim, j )
   enddo
   deallocate(u3s_tmp)
   deallocate(v3s_tmp)
   deallocate(t3s_tmp)
   ...
   ! store the state variable after calculating the advection, juanxiong he
   if(twoway_coupling.and.twoway_nudging.gt.0) then
       cam_state_ac=phys_state
   end if
...
!EOC
   ...
   ! store the tendency after calculating the advection, juanxiong he
   if(twoway_coupling.and.twoway_nudging.gt.0) then
       cam_tend_ac=phys_tend
   end if
...
subroutine stepon_run3( dtime, etamid, cam_out, phys_state, phys_tend,  &
...
!
   ...
   type(physics_state), intent(inout):: phys_state(begchunk:endchunk) !juanxiong he
   ...
   type(physics_tend), intent(inout) :: cam_tend(begchunk:endchunk) ! juanxionghe
   logical, intent(in) :: twoway_coupling           ! juanxiong he
...
!
  ...
  if(twoway_coupling) then  ! juanxiong he
     cam_state = phys_state
     cam_tend = phys_tend
  end if
