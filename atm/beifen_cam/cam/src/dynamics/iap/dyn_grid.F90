...
! Modified: Jiang Jinrong and Zhang He, 2012-10-25
! 
!-----------------------------------------------------------------------
...
! Author: Patrick Worley
! 
! Modified: Jiang Jinrong, OCT 2012
!-----------------------------------------------------------------------
#if ( defined SPMD )
...
!!             ns_vert(1,:nlon(1)) = -90.0_r8 
             ns_vert(1,:nlon(1)) = -90.0_r8  + (latdeg(1) - latdeg(2))*0.5_r8
          else
...
!!             ns_vert(2,:nlon(plat)) =  90.0_r8
             ns_vert(2,:nlon(plat)) =  90.0_r8 + (latdeg(plat) - latdeg(plat-1))*0.5_r8
          else
...
!#######################################################################
   ...
   integer function get_dyn_grid_parm(name) result(ival)
     use pmgrid, only : beglat, endlat, plat, plon, plev, plevp, &
          splon, beglev, endlev, endlevp, endlevp1, &
     ...
     if(name.eq.'splon') then
        ...
        ival = endlevp
     else if(name.eq.'endlevp1') then    !zhh
        ival = endlevp1
