...
! 2D parellel:  Jiang Jinrong, October 2012
!
!-----------------------------------------------------------------------
!
! $Id$
...
!-----------------------------------------------------------------------
   ...
   use pmgrid,       only: plon,plev,plevp,beglonxy,endlonxy
!-----------------------------------------------------------------------
   implicit none
!------------------------------Arguments--------------------------------
!
...
!
   real(r8), intent(in) :: f(beglonxy:endlonxy,pkdim)       ! constituent field
   real(r8), intent(in) :: lbasdz(4,2,pkdim)   ! vertical interpolation weights
!
! Output arguments
!
   real(r8), intent(out) :: dfz1(beglonxy:endlonxy,pkdim)    ! derivative at top of interval
   real(r8), intent(out) :: dfz2(beglonxy:endlonxy,pkdim)    ! derivative at bot of interval
!-----------------------------------------------------------------------
!
