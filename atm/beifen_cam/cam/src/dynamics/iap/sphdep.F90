...
!-----------------------------------------------------------------------
  ...
  use pmgrid
!!  ,       only: plev, plon, plat
  use scanslt,      only: platd, plond, beglatex, endlatex, i1, nxpt, nxptj, j1
...
!------------------------------Arguments--------------------------------
  ...
  real(r8), intent(in) :: ub (plond,beglev:endlev,beglatex:endlatex) ! x-deriv
  real(r8), intent(in) :: vb (plond,beglev:endlev,beglatex:endlatex) ! x-deriv
  real(r8), intent(in) :: uxl(plond,beglev:endlev,beglatex:endlatex) ! left  x-deriv (u)
  real(r8), intent(in) :: uxr(plond,beglev:endlev,beglatex:endlatex) ! right x-deriv 
  real(r8), intent(in) :: vxl(plond,beglev:endlev,beglatex:endlatex) ! left  x-deriv (v)
  real(r8), intent(in) :: vxr(plond,beglev:endlev,beglatex:endlatex) ! right x-deriv
  ...
  real(r8), intent(inout) :: lammp(plon,beglev:endlev)  ! long coord of midpoint
  real(r8), intent(inout) :: phimp(plon,beglev:endlev)  ! lat  coord of midpoint
  real(r8), intent(out)   :: lamdp(plon,beglev:endlev)  ! long coord of dep. point
  real(r8), intent(out)   :: phidp(plon,beglev:endlev)  ! lat  coord of dep. point
  integer , intent(out)   :: idp(plon,beglev:endlev,4)  ! long index of dep. point
  integer , intent(out)   :: jdp(plon,beglev:endlev)    ! lat  index of dep. point
!
...
!-----------------------------------------------------------------------
 ...
 !------------------------ local variables ------------------------------
  ...
  real(r8) upr  (plon,beglev:endlev)        ! u in local geodesic coords
  real(r8) vpr  (plon,beglev:endlev)        ! v in local geodesic coords
  real(r8) lampr(plon,beglev:endlev)        ! relative long coord of dep pt
  real(r8) phipr(plon,beglev:endlev)        ! relative lat  coord of dep pt
  real(r8) uvmp (plon,beglev:endlev,2)      ! u/v (spherical) interpltd to dep pt
  real(r8) fint (plon,beglev:endlev,ppdy,2) ! u/v x-interpolants
  real(r8) phidpmax
  real(r8) phidpmin
  real(r8) phimpmax
  real(r8) phimpmin
  real(r8) in(2,2),out(2,2)
!-----------------------------------------------------------------------
!
  do j=1,platd
...
!
     call herxin(1      ,1       ,ub      ,uxl   ,uxr          , &
                 lam    ,lammp   ,idp     ,jdp   ,fint(:,:,:,1), &
                 nlon   ,nlonex  )

     call herxin(1      ,1       ,vb      ,vxl   ,vxr          , &
                 lam    ,lammp   ,idp     ,jdp   ,fint(:,:,:,2), &
                 nlon   ,nlonex  )
...
!
     if ( locgeo ) then
        call s2gvel(uvmp(1,beglev,1),uvmp(1,beglev,2)   ,lam(i1,jcen) ,cphic ,sphic   , &
        ...
        call trjmps(dt      ,uvmp(1,beglev,1) ,uvmp(1,beglev,2),  phimp   ,lampr   , &
                    phipr   ,nlon    )
        finc = 1._r8
...
!JJR
        ...
        in(2,1)=imax
        in(2,2)=kmax
      call mpi_allreduce(in,out,2,MPI_2DOUBLE_PRECISION,MPI_MAXLOC,comm_z,ierr)
         imax=out(2,1)
         kmax=out(2,2)
         phimpmax=out(1,1)
      in(1,1)=phimpmin
      ...
      in(2,1)=imin
      in(2,2)=kmin
      call mpi_allreduce(in,out,2,MPI_2DOUBLE_PRECISION,MPI_MINLOC,comm_z,ierr)
       imin=out(2,1)
       kmin=out(2,2)
       phimpmax=out(1,1)
...
!
#if ( defined SPMD )
     if ( phimpmax >= phib(endlatex-nxptj) ) then   !zhh 2013-03-15
#else
     if ( phimp(imax,kmax) >= phib(j1+plat) ) then
#endif
        write(iulog,*)'SPHDEP: ****** MODEL IS BLOWING UP:  CFL condition likely violated-1 *********'
        ...
        call endrun
#if ( defined SPMD )
     else if( phimpmin < phib(beglatex+nxptj) ) then   !zhh 2013-03-15
#else
     else if( phimp(imin,kmin) < phib(j1-1) ) then
#endif
        write(iulog,*)'SPHDEP: ****** MODEL IS BLOWING UP:  CFL condition likely violated-2 *********'
...
!
#if ( defined SPMD )
     if ( phidpmax >= phib(endlatex-nxptj) ) then    !zhh 2013-03-15
#else
     if ( phidp(imax,kmax) >= phib(j1+plat) ) then
#endif
        write(iulog,*)'SPHDEP: ****** MODEL IS BLOWING UP:  CFL condition likely violated-3 *********'
        write(iulog,9000) imax,kmax,jgc
        write(iulog,*)' Possible solutions:  a)  reduce time step'
        write(iulog,*)'                      b)  if initial run, set "DIVDAMPN = 1." in namelist and rerun'
        write(iulog,*)'                      c)  modified code may be in error'
        call endrun
#if ( defined SPMD )
     else if( phidpmin < phib(beglatex+nxptj) ) then    !zhh 2013-03-15
#else
     else if( phidp(imin,kmin) < phib(j1-1) ) then
#endif
        write(iulog,*)'SPHDEP: ****** MODEL IS BLOWING UP:  CFL condition likely violated-4 *********'
        write(iulog,9000) imin,kmin,jgc
        write(iulog,*)' Possible solutions:  a)  reduce time step'
        write(iulog,*)'                      b)  if initial run, set "DIVDAMPN = 1." in namelist and rerun'
        write(iulog,*)'                      c)  modified code may be in error'
...
!-----------------------------------------------------------------------
  ...
  use shr_kind_mod, only: r8 => shr_kind_r8
  use pmgrid,       only: plon, plev, plat, beglev, endlev
...
!------------------------------Arguments--------------------------------
  ...
  real(r8), intent(in) :: upr  (plon,beglev:endlev)     ! u-wind in geodesic coord
  real(r8), intent(in) :: vpr  (plon,beglev:endlev)     ! v-wind in geodesic coord
  real(r8), intent(in) :: lamgc(plon,beglev:endlev)     ! geodesic long coord. of dep. point
  real(r8), intent(in) :: phigc(plon,beglev:endlev)     ! geodesic lat  coord. of dep. point
  integer , intent(in) :: nlon                 ! longitude dimension 
  real(r8), intent(out):: lamsc(plon,beglev:endlev)     ! spherical long coord. of dep. point
  real(r8), intent(out):: phisc(plon,beglev:endlev)     ! spherical lat  coord. of dep. point
!
...
!---------------------------Local variables-----------------------------
  ...
  integer nval(beglev:endlev)            ! number of values returned from whenfgt
  integer indx(plon,beglev:endlev)       ! index holder
  real(r8) pi                   ! 4.*atan(1.)
  real(r8) twopi                ! 2.*pi
  real(r8) pi2                  ! pi/2
  ...
  real(r8) slamgc(plon,beglev:endlev)    ! sin(lamgc)
  real(r8) dlam(plon,beglev:endlev)      ! zonal extent of trajectory
  real(r8) coeff                ! tmp variable
  real(r8) distmx               ! max distance
  real(r8) dist(plon,beglev:endlev)      ! approx. distance traveled along traj.
...
!-----------------------------------------------------------------------
  ...
  use shr_kind_mod, only: r8 => shr_kind_r8
  use pmgrid,       only: plon, beglev, endlev
...
!------------------------------Arguments--------------------------------
  ...
  real(r8), intent(in)  :: lamsc(plon,beglev:endlev)     ! spher. long coords of dep points
  real(r8), intent(in)  :: phisc(plon,beglev:endlev)     ! spher. lat  coords of dep points
  integer , intent(in)  :: nlon                 ! longitude dimension 
  real(r8), intent(out) :: phigc(plon,beglev:endlev)     ! loc geod. lat coords of dep points
!
! lam    longitude coordinates of the global grid points in spherical
!        system. The grid points in the global array are the reference
!        points for the local geodesic systems.
...
!------------------------------Arguments--------------------------------
  integer , intent(in) :: nlon                 ! longitude dimension 
  real(r8), intent(in) :: udp(plon,beglev:endlev)       ! u in spherical coords.
  real(r8), intent(in) :: vdp(plon,beglev:endlev)       ! v in spherical coords.
  real(r8), intent(in) :: lam(plond1)          ! x-coordinates of model grid
  real(r8), intent(in) :: cosphi               ! cos(latitude)
  real(r8), intent(in) :: sinphi               ! sin(latitude)
  real(r8), intent(in) :: lamdp(plon,beglev:endlev)     ! spherical longitude coord of dep pt.
  real(r8), intent(in) :: phidp(plon,beglev:endlev)     ! spherical latitude  coord of dep pt.
  real(r8), intent(out) :: upr(plon,beglev:endlev)      ! u in local geodesic coords.
  real(r8), intent(out) :: vpr(plon,beglev:endlev)      ! v in local geodesic coords.
!
...
!------------------------------Arguments--------------------------------
  integer , intent(in)    :: nlon             ! longitude dimension
  real(r8), intent(in)    :: dt               ! time step (seconds)
  real(r8), intent(in)    :: upr(plon,beglev:endlev)   ! u-component of wind in local geodesic
  real(r8), intent(in)    :: vpr(plon,beglev:endlev)   ! v-component of wind in local geodesic
  real(r8), intent(inout) :: phipr(plon,beglev:endlev) ! latitude coord of trajectory mid-point
  real(r8), intent(out)   :: lampr(plon,beglev:endlev) ! longitude coord of traj. mid-point
!
...
!------------------------------Arguments--------------------------------
  ...
  real(r8), intent(in) :: lampr(plon,beglev:endlev)     ! relative x-coordinate of departure pt.
  real(r8), intent(in) :: phipr(plon,beglev:endlev)     ! relative y-coordinate of departure pt.
  real(r8), intent(out) :: lamp (plon,beglev:endlev)    ! long coords of traj midpoints
  real(r8), intent(out) :: phip (plon,beglev:endlev)    ! lat  coords of traj midpoints
!
