...
!
! !USES:
  use clm_varpar  , only : numrad, ndst, nlevgrnd, nlevsno   !ndst = number of dust bins.
                                                          !nlevsno,  nlevgrnd, snow and ground level added by juanxiong he for wrf/cam coupling
...
!-------------------------------------------------------------------------------------- 
  real(r8), pointer :: soildepth(:,:)       ! soil depth (levgrnd, m) 
  real(r8), pointer :: soilthick(:,:)       ! soil thickness (levgrnd, m)
  real(r8), pointer :: soilt(:,:)       ! soil temperature (levgrnd, K)
  real(r8), pointer :: soilm(:,:)       ! soil column watert (levgrnd, m3/m3)
!--------------------------------------------------------------------------------------
! soildepth/height and soil temperature/moisture for wrf/cam coupling, added juanxiong he 
!-------------------------------------------------------------------------------------- 
...
! soildepth/height and soil temperature/moisture for wrf/cam coupling, added juanxiong he 
!--------------------------------------------------------------------------------------
        allocate(l2a%soildepth(beg:end,1:15 ))
        allocate(l2a%soilthick(beg:end,1:15 ))
        allocate(l2a%soilt(beg:end,1:15 ))
        allocate(l2a%soilm(beg:end,1:15 ))
        l2a%soildepth(beg:end, 1:15 ) = ival
        l2a%soilthick(beg:end, 1:15 ) = ival
        l2a%soilt(beg:end, 1:15 ) = ival
        l2a%soilm(beg:end, 1:15 ) = ival
!--------------------------------------------------------------------------------------
! soildepth/height and soil temperature/moisture for wrf/cam coupling, added juanxiong he 
!--------------------------------------------------------------------------------------
...
!-------------------------------------------------------------------------------------- 
  ...
  real(r8), pointer :: soilthick(:,:)       ! soil thickness (levgrnd, m)
  real(r8), pointer :: soilt(:,:)       ! soil temperature (levgrnd, K)
  real(r8), pointer :: soilm(:,:)       ! soil column watert (levgrnd, Kg/m2)
  real(r8) :: levdepth(15)       ! soil column watert (levgrnd, Kg/m2)
  data levdepth/0.007100635_8, 0.027925_8, 0.06225858_8, 0.1188651_8, 0.2121934_8, &
    0.3660658_8, 0.6197585_8, 1.038027_8, 1.727635_8, 2.864607_8, 4.739157_8, 7.829766_8, &
    12.92532_8, 21.32647_8, 35.17762_8/
!--------------------------------------------------------------------------------------
! soildepth/height and soil temperature/moisture for wrf/cam coupling, added juanxiong he 
!-------------------------------------------------------------------------------------- 
...
! soildepth/height and soil temperature/moisture for wrf/cam coupling, added juanxiong he 
!--------------------------------------------------------------------------------------
        allocate(soildepth(begg:endg,1:(nlevgrnd+nlevsno)))
        allocate(soilthick(begg:endg,1:(nlevgrnd+nlevsno)))
        allocate(soilt(begg:endg,1:(nlevgrnd+nlevsno)))
        allocate(soilm(begg:endg,1:(nlevgrnd+nlevsno))) 
  
        call c2g(begc, endc, begl, endl, begg, endg, nlevgrnd+nlevsno, &
          cptr%ces%t_soisno, soilt,&
          c2l_scale_type= 'urbanh', l2g_scale_type='unity')
        call c2g(begc, endc, begl, endl, begg, endg, nlevgrnd+nlevsno, &
          cptr%cws%h2osoi_vol, soilm,&
          c2l_scale_type= 'urbanh', l2g_scale_type='unity')
          
        do g=begg,endg
        do ig=1,nlevgrnd
        clm_l2a%soildepth(g,ig)= levdepth(ig)
        if (ig.gt.1) then
        clm_l2a%soilthick(g,ig)= levdepth(ig)-levdepth(ig-1)
        else
        clm_l2a%soilthick(g,ig)= levdepth(ig)
        end if
        clm_l2a%soilm(g,ig)=soilm(g,ig+nlevsno)
        clm_l2a%soilt(g,ig)=soilt(g,ig+nlevsno)
        enddo
        enddo
  
        deallocate(soildepth)
        deallocate(soilthick)
        deallocate(soilt)
        deallocate(soilm)
  
!        print *, clm_l2a%soilt(:,1)
