...
!-------------------------------------------------------------------------------------- 
  integer, dimension(1:15), public :: index_l2x_Sl_soildepth ! soil layer  depth
  integer, dimension(1:15), public :: index_l2x_Sl_soilthick ! soil layer  thickness
  integer, dimension(1:15), public :: index_l2x_Sl_soilt ! soil layer  temperature
  integer, dimension(1:15), public :: index_l2x_Sl_soilm ! soil layer  moisture
!--------------------------------------------------------------------------------------
! soildepth/height and soil temperature/moisture for wrf/cam coupling, added juanxiong he 
!--------------------------------------------------------------------------------------  
...
! !ARGUMENTS:
    implicit none
    character*2, dimension(1:nlevgrnd) :: slayer ! juanxiong he
...
!-----------------------------------------------------------------------
    ...
    do k = 1, nlevgrnd
    index_l2x_Sl_soildepth(k)   = mct_avect_indexra(l2x,'Sl_soildepth'//slayer(k) )
    index_l2x_Sl_soilthick(k)   = mct_avect_indexra(l2x,'Sl_soilthick'//slayer(k) )
    index_l2x_Sl_soilt(k)   = mct_avect_indexra(l2x,'Sl_soilt'//slayer(k) )
    index_l2x_Sl_soilm(k)   = mct_avect_indexra(l2x,'Sl_soilm'//slayer(k) )
    enddo
    !------------------------------------------------------------
    ! juanxiong he
    !------------------------------------------------------------
