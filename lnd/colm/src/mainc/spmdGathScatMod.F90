...
! !USES:
  ...
  use spmd, only : masterproc => p_master, npes => p_nprocs, iam => p_iam, mpicom => p_comm, MPI_INTEGER, MPI_REAL8
...
#endif
    use colm_varMod, only : numgrid, numpatch
    use spmd_decomp, only : numgrid_proc, numpatch_proc
!
! !ARGUMENTS:
    implicit none
...
!----------------------------------------------------------------------
    ...
    case('gridcell')
     ...
     ! indexi = begg
       ...
       numtot = numgrid * nfact
       do pid = 0,npes-1
          numperproc(pid) = numgrid_proc(pid) * nfact
       end do
       indexi = 1
    ...
    case('column')
     ...
     ! indexi = begc
       ...
       numtot = numpatch * nfact
       do pid = 0,npes-1
          numperproc(pid) = numpatch_proc(pid) * nfact
       end do
       indexi = 1
