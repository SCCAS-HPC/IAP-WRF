...
!-----------------------------------------------------------------------
    ...
    allocate(runoff%lnd(nsize), runoff%lnd_ixy(nsize), runoff%lnd_jxy(nsize), &
         runoff%lnd_area(nsize), stat=ier)
    if (ier /= 0) then
...
!-----------------------------------------------------------------------
    ...
    allocate(runoff%ocn(nsize), runoff%ocn_ixy(nsize), runoff%ocn_jxy(nsize), &
         runoff%ocn_area(nsize), stat=ier)
    if (ier /= 0) then
...
! !USES:
    use precision   , only : r8
    use spmd        , only : npes => p_nprocs, iam => p_iam
!
! !ARGUMENTS:
    implicit none
!
...
!-----------------------------------------------------------------------
    ...
    if (npes > nlnd) then
       write (6,*) 'set_proc_rof_bounds: number of processes exceeds number of land runoff cells', nlnd
       call endrun
    else if (npes > nocn) then
       write (6,*) 'set_proc_rof_bounds: number of processes exceeds number of ocean runoff cells', nocn
       call endrun
    end if
