#include <define.h>
 ...
 subroutine stomata (vmax25,effcon,slti,hlti,shti, &
                     ...
                     rb,ra,rstfac_r,cint,assim,respc,rst,&
#if (defined DGVM)
...
!     Revised by Ming in 05/10/2008
!     Last changed by Zhang Qian in 12/29/2008
!
...
!----------------------------------------------------------------------         
 ...
 real(r8),intent(in) :: &
      ...
      rstfac_r,     &! canopy resistance stress factors to soil moisture
...
#if (defined DGVM)
      assimpot,     &! canopy assimilation rate without water constrain                 
#endif
      respc,        &! virtual canopy respiration (mol m-2 s-1) 
...
!-------------------- local --------------------------------------------        
 ...
 real(r8)           &
      c3,           &! 1 for c3 vegetation; 0 for c4
      qt,           &! (tleaf - 298.16) / 10
      ...
      vmpot,        &! maximum catalytic activity of Rubison without water constrain
      jmax25,       &! potential rate of whole-chain electron transport at 25 C
      jmax,         &! maximum rate of whole-chain electron transport(mol electron m-2 s-1)
      jmaxpot,      &! maximum rate of whole-chain electron transport without water
                     ! constrain
      epar,         &! electron transport rate (mol electron m-2 s-1)
      eparpot,      &! electron transport rate for potential photosynthesis
      ...
      omsspot,      &! intermediate calculation for oms in potential photosynthesis
      ...
      assimn_pot,   &! net assimilation (mol co2 m-2 s-1)
      respc_pot,    &! canopy respiration (mol m-2 s-1) 
...
!----------------------------------------------------------------------         
! maximun capacity 
! vmpot  - maximum catalytic activity of Rubisco in the presence of     
!          saturating level of RuP2 and CO2 and without water constrain 
!          (mol m-2s-1)                                                 
! vm     - maximum catalytic activity of Rubisco in the presence of 
!          saturating level of RuP2 and CO2 with water constrain(mol m-2s-1)
...
! respc - virtual dark resipration used for canopy co2 concentration(mol m-2s-1)
...
!-----------------------------------------------------------------------        
      rstfac = rstfac_r
#if(defined DGVM)
      if (ivt >= 9 .and. ivt <= 11) then
         rstfac = min(1.0, rstfac_r/0.3) ! water stress for shrub, revised by zhq
      endif
#endif
      templ = 1. + exp(slti*(hlti-tlef))
      temph = 1. + exp(shti*(tlef-hhti))

      vmpot = vmax25 * 2.1**qt        ! (mol m-2 s-1)  
      vmpot = vmpot / temph * c3 + vmpot / (templ*temph) * (1.-c3) ! revised by zhq. dec29, 08
      vm = vmpot * rstfac !revised by zhq

      vmpot = vmpot * cint(1)                         
      vm = vm * cint (1)                             ! revised by zhq               
      rgas = 8.314                 ! universal gas constant (J mol-1 K-1)
!---> jmax25 = 2.39 * vmax25 - 14.2e-6        ! (mol m-2 s-1)
      jmax25 = 2.1 * vmax25        ! (mol m-2 s-1)
      jmaxpot = jmax25 * exp( 37.e3 * (tlef - trop) / (rgas*trop*tlef) ) * & 
             ( 1. + exp( (710.*trop-220.e3)/(rgas*trop) ) ) / &          
             ( 1. + exp( (710.*tlef-220.e3)/(rgas*tlef) ) )     !eq.33 Y.Dai,et.al, 2004 
                                   ! 37000  (J mol-1)
                                   ! 220000 (J mol-1)
                                   ! 710    (J K-1)
      jmax = jmaxpot * rstfac      ! revised by zhq.
      jmaxpot = jmaxpot *cint(2)
      jmax = jmax * cint(2)

      eparpot = min(4.6e-6 * par * effcon, 0.25*jmaxpot)
      epar = min(4.6e-6 * par * effcon, 0.25*jmax)

      respcp = 0.015 * c3 + 0.025 * (1. - c3) 
!      respc = respcp * vmax25 * 2.0**qt / ( 1. + exp( trda*(tlef-trdm )) ) * rstfac
      respc_pot = respcp * vmax25 * 2.0**qt / ( 1. + exp( trda*(tlef-trdm )) ) 
      respc = respc_pot * rstfac
      respc_pot = respc_pot * cint(1)
      respc = respc * cint(1)

      omsspot = ( vmax25/2. ) * (1.8**qt) / templ * c3 &
           + ( vmax25/5. ) * (1.8**qt) * (1. - c3) 
      omss = omsspot * rstfac
      omsspot = omsspot *cint(1)
      omss = omss * cint(1)
...
#if (defined DGVM)
! pco2i iteration for 'potential' photosynthesis
...
!begin iteration
      ...
      call sortin(eyy, pco2y, range, gammas, ic)      ! legacy from SiB2
      pco2i =  pco2y(ic)

      omc = vmpot * ( pco2i-gammas ) / ( pco2i + rrkk ) * c3 + vmpot * (1. - c3) 
      ome = eparpot * ( pco2i-gammas ) / ( pco2i+2.*gammas ) * c3 &
            + eparpot * (1. - c3) 
      oms   = omsspot * c3 + omsspot * pco2i * (1. - c3) 

      sqrtin= max( 0., ( (ome+omc)**2 - 4.*atheta*ome*omc ) ) 
      omp   = ( ( ome+omc ) - sqrt( sqrtin ) ) / ( 2.*atheta )
      sqrtin= max( 0., ( (omp+oms)**2 - 4.*btheta*omp*oms ) )
      assimpot = ( ( oms+omp ) - sqrt( sqrtin ) ) / ( 2.*btheta )

      assimn_pot = assimpot - respc_pot

      co2s = co2a - 1.37*assimn_pot/gbh2o                  ! mol mol-1

      co2st = min( co2s, co2a )
      co2st = max( co2st,1.e-5 )
                               
      assmt = max( 1.e-12, assimn_pot )
      hcdma = ei*co2st / ( gradm*assmt ) 

      aquad = hcdma                     
      bquad = gbh2o*hcdma - ei - binter*hcdma
      cquad = -gbh2o*( ea + hcdma*binter )  
                                             
      sqrtin= max( 0., ( bquad**2 - 4.*aquad*cquad ) )
      gsh2o = ( -bquad + sqrt ( sqrtin ) ) / (2.*aquad) 

      es  = ( gsh2o-binter ) * hcdma                   ! pa 
      es  = min( es, ei )
      es  = max( es, 1.e-2)

      gsh2o = es/hcdma + binter                        ! mol m-2 s-1                                      
      pco2in = ( co2s - 1.6 * assimn_pot / gsh2o )*psrf   ! pa

      eyy(ic) = pco2i - pco2in                        ! pa

      if( abs(eyy(ic)) .lt. 0.1 ) exit

      enddo              !iteration of assimilation without water constrain

#endif

!pco2i iteration for actual photosynthesis
...
! begin iteration
      ...
      omc = vm   * ( pco2i-gammas ) / ( pco2i + rrkk ) * c3 + vm * (1. - c3)
      ome = epar * ( pco2i-gammas ) / ( pco2i+2.*gammas ) * c3 + epar * (1. - c3)
      oms   = omss * c3 + omss*pco2i * (1. - c3)
...
!stomata conductance
      ...
      enddo               ! iteration of assimilation under water constrain 
...
!-----------------------------------------------------------------------        
      ...
      real(r8), INTENT(inout), dimension(6) :: eyy, pco2y                                             
!----- Local -----------------------------------------------------------        
...
3000  continue                                                                 
      ...
      pco2yl=pco2y(is) - (pco2y(is)-pco2y(isp))/(eyy(is)-eyy(isp))*eyy(is)     
