module tropause
contains
       SUBROUTINE  tropo(myid,temp, sx,ex,sy,ey,nlev, pres, plimu, pliml,&
                     plimlex, dofill, tp, tperr)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      implicit none
      integer                                        :: myid
      integer,intent(in)                             :: sx,ex,sy,ey,nlev
      real,intent(in),dimension(sx-1:ex+1,sy-1:ey+1,nlev) :: temp
      real,intent(in),dimension(sx-1:ex+1,sy-1:ey+1,nlev) :: pres
      real, intent(in)                          :: plimu, pliml, plimlex
      logical, intent(in)                       :: dofill
      real,intent(out),dimension(sx-1:ex+1,sy-1:ey+1)     :: tp
...
! check vertical orientation of data
      ...
      do lon=sx,ex
      do lat=sy,ey
       if (pres(lon,lat,1) .gt. pres(lon,lat,2)) then
       invert=1
       do i=1,nlev
        p(i)=pres(lon,lat,nlev+1-i)*100.  ! hPa > Pa
       enddo
       else
       invert=0
       do i=1,nlev
        p(i)=pres(lon,lat,i)*100.         ! hPa > Pa
       enddo
       endif
...
! fill holes
      ...
      do lon=sx,ex
       do lat=sy,ey
        tp(lon,lat) = max(plimlex,tp(lon,lat))
       enddo
      enddo  
return
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
       subroutine fill(dat, sx, ex, sy,ey,ir)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

       integer, intent(in)                  :: sx,ex,sy,ey
       integer, intent(out)                 :: ir
       real, dimension(sx-1:ex+1,sy-1:ey+1) :: dat
       ...
       enddo
       enddo
        if (icount.gt.((ex-sx+1)*(ey-sy+1))) then
            print*,icount,(ex-sx+1)*(ey-sy+1),'111111'
                stop 'ERROR: Too many holes (>50%)'
        endif   
         ir = icount
