...
subroutine geatm_final_mct()
   use geatm_vartype
!   if(allocated(procs)) deallocate (procs)  add ! by Wang Yuzhu

!   do ne=1,nest  add ! by Wang Yuzhu
!    call MPI_TYPE_FREE( stride(ne), ierr )  add ! by Wang Yuzhu
!    call MPI_COMM_FREE( comm2d(ne), ierr )  add ! by Wang Yuzhu
!   enddo  add ! by Wang Yuzhu
