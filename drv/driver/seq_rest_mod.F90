...
!EOP
   ...
   logical     :: wrf_present            ! .true.  => atm is present  juanxiong he
   logical     :: geatm_present            ! .true.  => atm is present  juanxiong he
   ...
   logical     :: wrf_prognostic         ! .true.  => atm comp expects input  juaxniong he
   logical     :: geatm_prognostic         ! .true.  => atm comp expects input  juaxniong he
...
!-------------------------------------------------------------------------------
   ...
   if (iamin_CPLID) then
      ...
      if (atm_present) then
         ...
         call seq_io_read(rest_file,gsmap,c2x_cx2,'c2x_cx')  ! juanxiong he
         call seq_cdata_setptrs(cdata_cax,gsmap=gsmap)  ! juanxiong he
         call seq_io_read(rest_file,gsmap,ca2x_cax2,'ca2x_cax')  ! juanxiong he
      endif
      if (wrf_present) then
         call seq_cdata_setptrs(cdata_mx,gsmap=gsmap)  ! juanxiong he
         call seq_io_read(rest_file,gsmap,m2x_mx,'m2x_mx')  ! juanxiong he
      endif
      if (geatm_present) then
         call seq_cdata_setptrs(cdata_gex,gsmap=gsmap)  ! juanxiong he
         call seq_io_read(rest_file,gsmap,chem2x_chemx,'chem2x_chemx')  ! juanxiong he
      endif
      if (lnd_present) then
