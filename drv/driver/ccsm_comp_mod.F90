...
!-------------------------------------------------------------------------------
   ...
   use shr_mpi_mod,       only: shr_mpi_gathScatVInit, shr_mpi_gatherv, shr_mpi_scatterv
   !  for transferring mgrid by huiqun hao
   use shr_kind_mod,      only: SHR_KIND_IN  !for globSize(:) in gather/scatter
   ...
   use perf_mod
   use ESMF_Mod
!   use wrap_mpi           ! for packing mgrid for direct ATM-WRF

   !----------------------------------------------------------------------------
   ! component model interfaces (init, run, final methods)
   !----------------------------------------------------------------------------
   use atm_comp_mct, only: atm_init_mct, atm_run_mct, atm_final_mct
   use atm_comp_mct, only: mgrid1, mgrid2, pack_mgrid  !huiqun hao
!   use coupling_utils, only: pack_mgrid
   ...
   !--- mapping routines ---
   use map_camcam_mct   ! juanxiong he ! huiqun hao used as atm2srd for srd
...
!  character(CL) :: hist_a2x24hr_flds = 'all'
   ...
   integer  :: nthreads_SRDID         ! OMP srd number of threads, huiqun hao for srd
   ...
   integer  :: nthreads_SRDATMID      ! OMP srd-atm number of threads, huiqun hao for srd
   integer  :: nthreads_SRDWRFID      ! OMP srd-wrf number of threads, huiqun hao for srd
   ...
   integer  :: mpicom_SRDID           ! MPI srd communicator, huiqun hao for srd
   ...
   integer  :: mpicom_SRDATMID        ! MPI srd-atm communicator, huiqun hao for srd
   integer  :: mpicom_SRDWRFID        ! MPI srd-wrf communicator, huiqun hao for srd
   ...
   logical  :: iamroot_SRDID          ! SRDID masterproc, huiqun hao for srd
   ...
   logical  :: iamin_SRDID            ! pe associated with SRDID, huiqun hao for srd
...
!   logical  :: iamin_CPLWRFID         ! pe associated with CPLWRFID, juanxiong he
   ...
   logical  :: iamin_SRDATMID         ! pe associated with SRDATMID, huiqun hao for srd
   logical  :: iamin_SRDWRFID         ! pe associated with SRDWRFID, huiqun hao for srd

   character(CL) :: complist          ! list of comps on this pe
   integer  :: iam_GLOID              ! pe number in global id
   
!for mgrid transferring , huiqun hao*****************************************************
   integer  :: iam_SRDID              ! pe number in SRD id, huiqun hao
   integer  :: iam_SRDATMID           ! pe number in SRDATM id, huiqun hao
   real(r8), pointer :: glob1DArr(:)!, glob1DArrBack(:)
!   integer,  pointer :: globSize(:), displs(:)
!   integer :: num_SRDATMID
   integer :: root_ATMSRD
   integer :: myrank_ATMSRD
!   real(r8), pointer,  dimension(:,:) :: mgrid1_xlat
!   character         :: atm_2_wrf
   integer           :: atm_2_wrf
!   real(r8),pointer  :: mgrid1_all    ! Global 1D array of gathered data
!   real(r8) ::  mgrid_test
!   integer(sin),pointer  :: mgrid1_size   ! Size of each distributed piece
!   integer(sin)      :: locSize       ! Number to send this PE
!   character(*)      :: send_mgrid   = 'mgrid'
!for mgrid transferring , huiqun hao*****************************************************
   ...
   !--------------------------------------------------------------------------
    ...
    if (Global_Comm /= MPI_COMM_NULL) then
     call seq_comm_init(Global_Comm, NLFileName, atm_petlist=atm_petlist, lnd_petlist=lnd_petlist, &
           ...
           wrf_petlist=wrf_petlist, gea_petlist=gea_petlist, srd_petlist=srd_petlist) ! juanxiong he
    end if
    call seq_io_init2()
...
!   write(logunit,*) ' before SRDID comm set up ',SRDID
   call seq_comm_setptrs(SRDID,mpicom=mpicom_SRDID,iamroot=iamroot_SRDID,nthreads=nthreads_SRDID) ! huiqun hao for srd
...
!   write(logunit,*) ' before SRDATMID comm set up ',SRDATMID
   call seq_comm_setptrs(SRDATMID,mpicom=mpicom_SRDATMID,nthreads=nthreads_SRDATMID) !huiqun hao for srd
!   write(logunit,*) ' before SRDWRFID comm set up ',SRDWRFID
   call seq_comm_setptrs(SRDWRFID,mpicom=mpicom_SRDWRFID,nthreads=nthreads_SRDWRFID) !huiqun hao for srd
!   write(logunit,*) ' after all comms set up ','SRDID',SRDID,'SRDATMID',SRDATMID,'SRDWRFID',SRDWRFID
!   write(202,*) 'mpicom_SRDID',mpicom_SRDID,'mpicom_SRDATMID',mpicom_SRDATMID,'mpicom_SRDWRFID',mpicom_SRDWRFID
!   write(203,*) 'mpicom_ATMID',mpicom_ATMID,'mpicom_CPLATMID',mpicom_CPLATMID,'mpicom_CPLWRFID',mpicom_CPLWRFID
   ...
   iamin_SRDID    = seq_comm_iamin(SRDID) ! huiqun hao for srd
...
!   iamin_CPLWRFID = seq_comm_iamin(CPLWRFID) !juanxiong he
   ...
   iamin_SRDATMID = seq_comm_iamin(SRDATMID) !huiqun hao for srd
   iamin_SRDWRFID = seq_comm_iamin(SRDWRFID) !huiqun hao for srd
!   write(logunit,*) 'iamin_CPLATMID',iamin_CPLATMID ! correct 160responding 64 T 96 F
!   write(logunit,*) 'iamin_SRDATMID',iamin_SRDATMID ! correct 160responding 96 T 64 F
!   write(logunit,*) 'iamin_SRDWRFID',iamin_SRDWRFID ! correct 160responding 96 T 64 F
!  checked correct
!   write(logunit,*) 'iamroot_CPLID',iamroot_CPLID
!   write(logunit,*) 'iamroot_ATMID',iamroot_ATMID
!   write(logunit,*) 'iamroot_SRDID',iamroot_SRDID
!   write(logunit,*) 'iamroot_WRFID',iamroot_WRFID
   complist = " "
   ...
   if (iamin_SRDID) complist = trim(complist)//' srd' !huiqun hao for srd
...
!   write(logunit,*) 'iamroot_WRFID',iamroot_WRFID
   ...
   if (drv_threading) then
      ...
      if (iamroot_GLOID) write(logunit,*)
	  call seq_comm_setnthreads(nthreads_SRDID) ! huiqun hao
      if (iamroot_GLOID) write(logunit,'(2A,2I4)') subname,'    nthreads_SRDID = ',nthreads_SRDID,seq_comm_getnthreads()
      if (iamroot_GLOID) write(logunit,*)
      !not responding 20160802
      call seq_comm_setnthreads(nthreads_GLOID)
   endif
   write(logunit,*) ' check AAA ',CPLID,iamroot_GLOID,iamroot_CPLID,iamroot_SRDID !20160728
   !-----------------------------------------------------------------------------
...
!   call seq_cdata_init(cdata_cc, ATMID, dom_cc, gsMap_cc, infodata, 'cdata_cc' )   !juanxiong he, wrf/cam coupling, cam end
   ...
   call seq_cdata_init(cdata_mx, SRDID, dom_mx, gsMap_mx, infodata, 'cdata_mx' )  !huiqun hao, srd coupling, wrf end on srd pes
   call seq_cdata_init(cdata_cx, SRDID, dom_cx, gsMap_cx, infodata, 'cdata_cx' )  !huiqun hao, srd coupling, cam end on srd pes
   call seq_cdata_init(cdata_mm, WRFID, dom_mm, gsMap_mm, infodata, 'cdata_mm' )  !huiqun hao, srd coupling, wrf end on wrf pes
   call seq_cdata_init(cdata_cc, ATMID, dom_cc, gsMap_cc, infodata, 'cdata_cc' )  !huiqun hao, srd coupling, cam end on cam pes

   !-----------------------------------------------------------------------------
   ! Initialize time manager
   !-----------------------------------------------------------------------------
   call mpi_barrier(mpicom_GLOID,ierr)
   write(logunit,*) ' check BBB ',CPLID,iamroot_GLOID,iamroot_CPLID,iamroot_SRDID !20160728
   ...
   if (iamroot_CPLID) then
	   ...
	   write(logunit,*) ' check BBBinsideif ',CPLID,iamroot_GLOID,iamroot_CPLID,iamroot_SRDID !20160728
   endif
   ...
   if ( iamroot_CPLID )then
      write(logunit,*) ' '
      write(logunit,F00) 'Initialize each component: atm, gea, lnd, ocn, and ice' ! juanxiong he
      call shr_sys_flush(logunit)
   endif
   
!   if ( iamroot_SRDID )then
!      write(logunit,*) ' '
!      write(logunit,F00) 'Initialize each component: atm, wrf' ! huiqun hao
!      call shr_sys_flush(logunit)
!   endif
   ...
   !-----------------------------------------------------------------------------
   call mpi_barrier(mpicom_GLOID,ierr)
   write(logunit,*) ' check CCC ',iamin_CPLATMID,iamroot_GLOID,iamroot_CPLID !20160728
   ...
   !-----------------------------------------------------------------------------
   ! Initialization atmospheric component for srd coupling !huiqun hao for srd
   !-----------------------------------------------------------------------------
   call mpi_barrier(mpicom_GLOID,ierr)
   write(logunit,*) ' check DDD ',iamin_SRDATMID !96 pe responce, 32 T 20160728
   if (iamin_SRDATMID) then
      call seq_infodata_exchange(infodata,SRDATMID,'cpl2atm_init')
   endif
   if (iamin_SRDID .and. atm_present) then
      if (seq_comm_iamroot(ATMID)) write(logunit,F00) 'Initialize atm component'
      call shr_sys_flush(logunit)
      if (drv_threading) call seq_comm_setnthreads(nthreads_ATMID)
      call atm_init_mct( EClock_a, cdata_aa, cdata_cc, cdata_caca, x2a_aa, a2x_aa, &
   ...
   if (iamin_SRDATMID) then
      call seq_infodata_exchange(infodata,SRDATMID,'atm2cpl_init')
   endif

   !-----------------------------------------------------------------------------
   ! Initialization WRF component, juanxiong he ! huiqun hao altered for srd
   !-----------------------------------------------------------------------------   
   call mpi_barrier(mpicom_GLOID,ierr)
   write(logunit,*) ' check EEE ',iamin_SRDWRFID !64 pe responce 64 T?? 20160728
   if (iamin_SRDWRFID) then
      call seq_infodata_exchange(infodata,SRDWRFID,'cpl2wrf_init')
   endif
   ...
   if (iamin_SRDWRFID) then
      call seq_infodata_exchange(infodata,SRDWRFID,'wrf2cpl_init')
   endif

   !-----------------------------------------------------------------------------
   ! Initialization GEATM component, juanxiong he
   !-----------------------------------------------------------------------------   
   write(logunit,*) ' check FFF ',iamin_CPLGEAID !20160728
   ...
   if (iamin_SRDATMID) then !huiqun hao for srd coupling
      call seq_infodata_exchange(infodata,SRDATMID,'cpl2atm_init')
   endif
!   if (iamin_CPLWRFID) then  !juanxiong he
!      call seq_infodata_exchange(infodata,CPLWRFID,'cpl2wrf_init')
!   endif
   if (iamin_SRDWRFID) then  !huiqun hao for srd coupling
      call seq_infodata_exchange(infodata,SRDWRFID,'cpl2wrf_init')
   endif
...
!      call mct_accum_init(x2oacc_ox, x2o_ox)
   ...
   !-----------------------------------------------------------------------------
   ! GEATM and WRF, Juanxiong He !altered by huiqun hao for srd coupling
   !-----------------------------------------------------------------------------
!   write(204,*) 'mpicom_CPLATMID',mpicom_CPLATMID,'mpicom_SRDATMID',mpicom_SRDATMID
!maybedel
...
!   endif
   ...
   if (iamin_SRDATMID .and. atm_present) then !huiqun hao, for srd coupling, cam import
!tmpdel
      
	  call pack_mgrid(mgrid1, atm_2_wrf, mpicom_SRDATMID)
!	  write(204,*) 'atm_2_wrf',atm_2_wrf
	  if (iamroot_SRDID) then
	      call MPI_gather(atm_2_wrf, 1, MPI_PACKED, atm_2_wrf, 0, MPI_PACKED, 64, mpicom_SRDATMID)
	  endif !ADDSRD
!tmpdel
!	  call mpi_comm_rank(mpicom_SRDATMID,iam_SRDATMID)
!	  if (iam_SRDATMID==0)write(202,*) mgrid1
!	  if (iamroot_SRDID) then
!	      call mpi_comm_rank(SRDID,iam_SRDID)
!		  call mpi_comm_rank(SRDATMID,iam_SRDATMID)
!	  endif
!	  write(202,*) 'SRDATMID',SRDATMID
!	  write(203,*) 'iam_SRDATMID',iam_SRDATMID
!	  write(202,*) 'mgrid_test',mgrid_test
!	  write(205,*) 'mgrid1_all'
!	  write(206,*) 'mgrid1_Size'
!      num_SRDATMID = SRDATMID
!	  root_ATMSRD  = 0
!	  mgrid1_xlat = mgrid1%xlat
!	  write(202,*) 'num_SRDATMID',num_SRDATMID
!	  write(203,*) 'root_ATMSRD',root_ATMSRD
!	  write(202,*) 'mgrid1_xlat',mgrid1_xlat
!	  write(205,*) 'glob1DArr',glob1DArr
!	  write(206,*) 'globSize',globSize
!	  write(207,*) 'displs',displs
!      call MPI_gather(mgrid1, 1, MPI_INT, mgrid1, reccount, MPI_INT, source, SRDATMID)
!	  call shr_mpi_gathScatvInit(SRDATMID, root_ATMSRD, mgrid1, glob1DArr, globSize, displs )
!	  call shr_mpi_gathScatVInit(SRDATMID, iam_SRDATMID, mgrid_test, mgrid1_all, mgrid1_Size, 0, 'send_mgrid')
!	  if (iamroot_SRDID) then
!	      call shr_mpi_gatherv(mgrid1, locSize, mgrid1_all, mgrid1_Size, 0, iam_SRDATMID, SRDATMID)
	  endif
!TAG0218
!      call mct_avect_zero(x2c_cc1)
!      call mct_avect_zero(x2c_cc2)
...
!   endif
   ...
   if (iamin_SRDWRFID .and. wrf_present) then !huiqun hao, for srd coupling, wrf import
!      call map_wrf2wrf_init_mct(cdata_mm, x2m_mm1, m2x_mm, WRFID, &
!                                cdata_mx, x2m_mx1, m2x_mx, SRDID, SRDWRFID)
!      call map_wrf2wrf_init_mct(cdata_mm, x2m_mm2, m2x_mm, WRFID, &
!                                cdata_mx, x2m_mx2, m2x_mx, SRDID, SRDWRFID)
!	  if (iamroot_SRDID) then
!	      call mpi_comm_rank(SRDID,iam_SRDID)
!		  call mpi_comm_rank(SRDATMID,iam_SRDATMID)
!	  endif
!      call mpi_comm_rank(mpicom_SRDWRFID , myrank_ATMSRD)
!tmpdel
	  if (iamroot_SRDID) then
	      call MPI_Bcast(mgrid1, 1, MPI_PACKED, 0, mpicom_SRDWRFID)
	  endif  !ADDSRD
!tmpdel
!	  call shr_mpi_gathScatVInitr(SRDWRFID, iam_SRDWRFID, mgrid1, glob1DArr, globSize, &
!                                   displs, string )
!TAG0218
!      call mct_avect_zero(x2m_mm1)
!      call mct_avect_zero(x2m_mm2)
...
!del hhq            call map_cam2wrf_mct( cdata_cx, c2x_cx1, cdata_mx, c2x_mx1 , &
!                                    statelist=seq_flds_c2x_states ) ! juanxiong he
!del hhq            call map_cam2wrf_mct( cdata_cx, c2x_cx2, cdata_mx, c2x_mx2 , &
!                                    statelist=seq_flds_c2x_states ) ! juanxiong he
!del hhq            call mrg_x2w_run_mct( cdata_mx, c2x_mx1, x2m_mx1)  ! juanxiong he
!del hhq            call mrg_x2w_run_mct( cdata_mx, c2x_mx2, x2m_mx2)  ! juanxiong he
...
subroutine ccsm_run()
 ...
 106  format( A, f23.12)
   ...
   !----------------------------------------------------------
   write(logunit,*) ' check hhq ccsm_run 1854'
   call t_startf ('DRIVER_RUN_LOOP_BSTART')
!   write(logunit,*) 'AAA', mpicom_GLOID
   call mpi_barrier(mpicom_GLOID,ierr)
!   write(logunit,*) 'BBB', mpicom_GLOID
   call t_stopf ('DRIVER_RUN_LOOP_BSTART')
   write(logunit,*) ' check hhq ccsm_run 1860'
   ...
   do while ( .not. stop_alarm)
      ...
      write(logunit,*) ' check hhq ccsm_run 1902'
      ...
      write(logunit,*) ' check hhq ccsm_run 1937'
      if (iamroot_CPLID) then
         if (loglevel > 1) then
...
!                                  fluxlist=seq_flds_xao_fluxes, statelist=seq_flds_xao_states)
      ...
      write(logunit,*) ' check hhq ccsm_run 2514'
      !----------------------------------------------------------
      ! ATM SETUP
      !----------------------------------------------------------
...
!	  write(204,*) 'atm_2_wrf',atm_2_wrf
	  ...
	  print *, 'before 1st atm_run_mct 2600'
      ...
      if (atm_present .and. atmrun_alarm .and. iamin_ATMID) then
		 ...
		 !**********************************************************
		 !pack mgrid for transmiting Huiqun Hao
		 !call pack_mgrid(mgrid1, atm_2_wrf, mpicom_SRDATMID)
		 !if (iamroot_ATMID) write (204,*) 'atm_2_wrf',atm_2_wrf
		 !call MPI_gather(atm_2_wrf, 1, MPI_PACKED, mgrid1, 0, MPI_PACKED, 64, mpicom_SRDATMID)
		 !**********************************************************
         if (drv_threading) call seq_comm_setnthreads(nthreads_GLOID)
         call t_drvstopf  ('DRIVER_ATM_RUN')
      endif
	  print *, 'after 1st atm_run_mct 2600'
      ...
      !----------------------------------------------------------
      ! atm -> srd for wrf, juanxiong he ! altered by huiqun hao for srd coupling
      !----------------------------------------------------------
	  write(logunit,*) ' before check 2689 hhq'
      if (atm_present .and. atmrun_alarm) then
         if (iamin_SRDATMID) then
            call t_drvstartf ('DRIVER_A2C',barrier=mpicom_SRDATMID)
            call t_drvstartf ('driver_a2c_atma2atmx',barrier=mpicom_SRDATMID)
			call pack_mgrid(mgrid1, atm_2_wrf, SRDATMID)
	        if (iamroot_SRDID) then
	              call MPI_gather(atm_2_wrf, 1, MPI_PACKED, mgrid1, 64, MPI_PACKED, 64, SRDATMID)
	        endif !ADDSRD
!maybedel
!            call map_cama2camx_mct(cdata_cc, c2x_cc1, cdata_cx, c2x_cx1)  !juanxiong he
!            call map_cama2camx_mct(cdata_cc, c2x_cc2, cdata_cx, c2x_cx2)  !juanxiong he
!TAG
!srd************************************************************************************
!            call dc_cama2camx_srd(mgrid1_aa, mgrid1_xa)   !Huiqun Hao
!			call dc_cama2camx_srd(mgrid2_aa, mgrid2_xa)   !Huiqun Hao
!srd************************************************************************************
            call t_drvstopf  ('driver_a2c_atma2atmx')
            call t_drvstartf ('driver_a2c_infoexch',barrier=mpicom_SRDATMID)
            call seq_infodata_exchange(infodata,SRDATMID,'atm2cpl_run')
            call t_drvstopf  ('driver_a2c_infoexch')
            call t_drvstopf  ('DRIVER_A2C')
         endif
      endif ! run alarm
	  write(logunit,*) ' after check 2689 hhq'
     ...
     if (wrf_present .and. wrfrun_alarm) then
         ...
         if (iamin_SRDID) then
         if (drv_threading) call seq_comm_setnthreads(nthreads_SRDID)
         call t_drvstartf ('DRIVER_WRFPREP',cplrun=.true.,barrier=mpicom_SRDID)

         call t_drvstartf ('driver_wrfprep_cam2wrf',barrier=mpicom_SRDID)
...
!TAG
!srd************************************************************************************
!            call dc_cam2wrf_srd( mgrid1_xa, mgrid1_xw)   !Huiqun Hao
!			call dc_cam2wrf_srd( mgrid2_xa, mgrid2_xw)   !Huiqun Hao
!srd************************************************************************************
         call t_drvstopf  ('driver_wrfprep_cam2wrf')

         if ( seq_comm_iamroot(SRDID)) write(logunit,F00) 'Calling mrg_x2w_run_mct'
!maybedel
...
!         call mrg_x2w_run_mct( cdata_mx, c2x_mx2, x2m_mx2)
         ...
         if (iamin_SRDWRFID .and. wrf_prognostic) then
            call t_drvstartf ('DRIVER_C2W',barrier=mpicom_SRDWRFID)
            call t_drvstartf ('driver_c2w_wrfx2wrfa',barrier=mpicom_SRDWRFID)
!tmpdel			
!			call mpi_comm_rank(mpicom_SRDWRFID , myrank_ATMSRD)
!	        if (myrank_ATMSRD == 0) then
!	             call MPI_Bcast(mgrid1, 1, MPI_PACKED, 0, mpicom_SRDWRFID)
!	        endif !ADDSRD
!tmpdel
!maybedel
...
!TAG
!srd************************************************************************************
!            call dc_wrfx2wrfw_srd( mgrid1_xw, mgrid1_ww)   !Huiqun Hao
!			call dc_wrfx2wrfw_srd( mgrid2_xw, mgrid2_ww)   !Huiqun Hao
!srd************************************************************************************
            call t_drvstopf  ('driver_c2w_wrfx2wrfa')
            call t_drvstartf ('driver_c2w_infoexch',barrier=mpicom_SRDWRFID)
            call seq_infodata_exchange(infodata,SRDWRFID,'cpl2wrf_run')
...
!del hhq               call map_wrf2cam_mct( cdata_mx, m2x_mx, cdata_cx, m2x_cx , &
