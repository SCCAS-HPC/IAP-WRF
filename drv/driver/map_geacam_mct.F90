...
!--------------------------------------------------------------------------
  ...
  public :: map_cam2gea_init_mct
  public :: map_gea2cam_init_mct
  public :: map_cam2gea_mct
  public :: map_gea2cam_mct
...
!--------------------------------------------------------------------------
  ...
  type(mct_rearr), private :: Re_cam2gea
  type(mct_rearr), private :: Re_gea2cam
  type(mct_sMatp), private :: sMatp_Fge2c
  type(mct_sMatp), private :: sMatp_Sge2c
  type(mct_sMatp), private :: sMatp_Fc2ge
  type(mct_sMatp), private :: sMatp_Sc2ge
...
#endif
  logical, private :: samegrid_mapge2c

!=======================================================================
   contains
!=======================================================================

  subroutine map_gea2cam_init_mct( cdata_w, cdata_c)
    ...
    character(*),parameter :: subName = '(map_gea2cam_init_mct) '
    !--------------------------------------------------
    ...
    call seq_infodata_GetData( infodata, samegrid_gea=samegrid_mapge2c)

    if (samegrid_mapge2c) then

       call mct_rearr_init(gsMap_w, gsMap_c, mpicom, Re_gea2cam)

    else

       call shr_mct_sMatPInitnc(sMatp_Fge2c,gsMap_w,gsMap_c,"seq_maps.rc", &
          "gea2camFmapname:","gea2camFmaptype:",mpicom)

       call shr_mct_sMatPInitnc(sMatp_Sge2c,gsMap_w,gsMap_c,"seq_maps.rc", &
          "gea2camSmapname:","gea2camSmaptype:",mpicom)

    endif

  end subroutine map_gea2cam_init_mct

!=======================================================================

  subroutine map_cam2gea_init_mct( cdata_c, cdata_w )
    ...
    integer                  :: mpicom            ! communicator spanning atm and lnd
    character(*),parameter :: subName = '(map_cam2gea_init_mct) '
    !--------------------------------------------------
    ...
    if (samegrid_mapge2c) then
       ...
       call mct_rearr_init(gsMap_c, gsMap_w, mpicom, Re_cam2gea)

    else

       call shr_mct_sMatPInitnc(sMatp_Fc2ge, gsMap_c, gsMap_w, "seq_maps.rc", &
            "cam2geaFmapname:", "cam2geaFmaptype:", mpicom)

       call shr_mct_sMatPInitnc(sMatp_Sc2ge, gsMap_c, gsMap_w, "seq_maps.rc", &
            "cam2geaSmapname:", "cam2geaSmaptype:", mpicom)

    endif

  end subroutine map_cam2gea_init_mct

!=======================================================================

  subroutine map_gea2cam_mct( cdata_w, av_w, cdata_c, av_c, fluxlist, statelist )
    ...
    character(*),parameter :: subName = '(map_gea2cam_mct) '
    !--------------------------------------------------
    ...
    if (samegrid_mapge2c) then
       ...
       if (present(fluxlist) .or. present(statelist)) then
          if (present(fluxlist)) then
             call mct_rearr_rearrange_fldlist(av_w, av_c, Re_gea2cam, VECTOR=usevector, &
          ...
          call mct_rearr_rearrange(av_w, av_c, Re_gea2cam, VECTOR=usevector, ALLTOALL=usealltoall)
       end if
       ...
       if (present(fluxlist) .or. present(statelist)) then
          if (present(fluxlist)) then
             ...
             call mct_sMat_avMult(av_w, sMatp_Fge2c, av_c_f, VECTOR=usevector, rList=fluxlist)
          ...
          if (present(statelist)) then
             ...
             call mct_sMat_avMult(av_w, sMatp_Sge2c, av_c_s, VECTOR=usevector, rList=statelist)
             call mct_aVect_copy (aVin=av_c_s, aVout=av_c, vector=usevector)
             call mct_aVect_clean(av_c_s)
          end if
       else
          !--- default is flux mapping
          call mct_sMat_avMult(av_w, sMatp_Fge2c, av_c, VECTOR=usevector)
       endif
    endif

!    lsize = mct_avect_lsize(av_c)  ! juanxiong he, debug
!    do n = 1,lsize
!      if(av_w%rAttr(index_w2x_Sw_ps,n).ne.0) print *, av_w%rAttr(index_w2x_Sw_ps,n), samegrid_mapge2c 
!    end do
       
  end subroutine map_gea2cam_mct

!=======================================================================

  subroutine map_cam2gea_mct( cdata_c, av_c, cdata_w, av_w, &
    ...
    character(*),parameter :: subName = '(map_cam2gea_mct) '
    !--------------------------------------------------
    ...
    if (samegrid_mapge2c) then
       ...
       if (present(fluxlist) .or. present(statelist)) then
          if (present(fluxlist)) then
             call mct_rearr_rearrange_fldlist(av_c, av_w, Re_cam2gea, VECTOR=usevector, &
                  ALLTOALL=usealltoall, fldlist=fluxlist)
          endif
          if (present(statelist)) then
             call mct_rearr_rearrange_fldlist(av_c, av_w, Re_cam2gea, VECTOR=usevector, &
          ...
          call mct_rearr_rearrange(av_c, av_w, Re_cam2gea, VECTOR=usevector, ALLTOALL=usealltoall)
       end if
          ...
          if (present(fluxlist) .or. present(statelist)) then
             if (present (fluxlist)) then
                ...
                call mct_sMat_avMult(av_c, sMatp_Fc2ge, av_w_f, VECTOR=usevector, rList=fluxlist )
             ...
             if (present(statelist)) then
                ...
                call mct_sMat_avMult(av_c, sMatp_Sc2ge, av_w_s, VECTOR=usevector, rList=statelist)
                call mct_aVect_copy (aVin=av_w_s, aVout=av_w, vector=usevector)
                call mct_aVect_clean(av_w_s)
             end if
          else
             ! --- default is flux mapping
             call mct_sMat_avMult(av_c, sMatp_Fc2ge, av_w, VECTOR=usevector)
          endif

    endif

  end subroutine map_cam2gea_mct

end module map_geacam_mct
