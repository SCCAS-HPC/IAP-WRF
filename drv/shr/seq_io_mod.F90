!===============================================================================
! SVN $Id: seq_io_mod.F90 28202 2011-04-29 23:30:47Z tcraig $
! SVN $URL: https://svn-ccsm-models.cgd.ucar.edu/drv/seq_mct/branch_tags/t3148b_tags/t3148b02_drvseq3_1_48/shr/seq_io_mod.F90 $
!===============================================================================
!BOP ===========================================================================
!
...
module seq_io_mod
  ...
  use seq_comm_mct, only : logunit, seq_comm_setptrs, CPLID, LNDID, ATMID, OCNID, ICEID, GLCID, GEAID ! juanxiong he
...
!-------------------------------------------------------------------------------
   ...
   integer, parameter :: io_components=7  ! juanxiong he
   integer, parameter :: io_compid(io_components) = (/CPLID, ATMID, ICEID, OCNID, LNDID, GLCID, GEAID/) ! juanxiong he
   character(len=3), parameter :: io_compname(io_components) = (/'CPL','ATM','ICE','OCN','LND','GLC','GEA'/) ! juanxiong he
...
! !INTERFACE: --------------------------------------------------------------------
  ...
  subroutine seq_io_init1(nlfilename, Global_Comm)
    ...
    character(CS) :: gea_pio_typename ! juanxiong he
    ...
    integer(IN)    :: gea_pio_iotype ! juanxiong he 
    integer(IN)    :: gea_pio_stride ! juanxiong he
    integer(IN)    :: gea_pio_numtasks ! juanxiong he
    integer(IN)    :: gea_pio_root ! juanxiong he
    ...
    namelist /pio_inparm/ pio_stride, pio_root, pio_numtasks, pio_typename, pio_async_interface, pio_debug_level, &
         ...
         gea_pio_stride, gea_pio_root, gea_pio_numtasks, gea_pio_typename  ! juanxiong he
    ...
    gea_pio_stride   = -99 ! juanxiong he
    gea_pio_numtasks = -99 ! juanxiong he0
    gea_pio_root     =  -99  ! juanxiong he0
    gea_pio_typename = 'nothing' ! juanxiong he0
	...
	call shr_sys_abort(subname//':: async IO requires an MPI2 compliant MPI library')
       ...
       if(iamroot) then
          do i=1,io_components
             ...
             else if(io_compname(i)=='GEA') then  ! juanxiong he
                comp_pio_stride(i)   = gea_pio_stride
                comp_pio_numtasks(i) = gea_pio_numtasks
                comp_pio_root(i)     = gea_pio_root
                call getiotypefromname(gea_pio_typename, comp_pio_iotype(i),pio_iotype)
