!===============================================================================
! SVN $Id: seq_timemgr_mod.F90 28202 2011-04-29 23:30:47Z tcraig $
! SVN $URL: https://svn-ccsm-models.cgd.ucar.edu/drv/seq_mct/branch_tags/t3148b_tags/t3148b02_drvseq3_1_48/shr/seq_timemgr_mod.F90 $
!===============================================================================
!BOP ===========================================================================
!
...
!EOP
   ...
   character(len=*),public,parameter :: &
      ...
      seq_timemgr_clock_wrf  = 'seq_timemgr_clock_wrf' , &  ! juanxiong he for wrf/ccsm4 coupling
      seq_timemgr_clock_gea  = 'seq_timemgr_clock_gea'   ! juanxiong he for geatm/ccsm4 coupling
   integer(SHR_KIND_IN),private,parameter :: &
      ...
      seq_timemgr_nclock_wrf  = 7, &       ! by juanxiong he for wrf/cam coupling
      seq_timemgr_nclock_gea  = 8       ! by juanxiong he for geatm/cam coupling
   character(len=8),private,parameter :: seq_timemgr_clocks(max_clocks) = &
      (/'drv     ','atm     ','lnd     ','ocn     ', &
        'ice     ','glc     ','wrf     ','gea     '/) ! by juanxiong he for wrf/cam coupling

   integer(SHR_KIND_IN),private,parameter :: max_alarms = 15  ! by juanxiong he
   character(len=*),public,parameter :: &
      ...
      seq_timemgr_alarm_wrfrun  = 'seq_timemgr_alarm_wrfrun  ', &  ! by juanxiong he for wrf/cam coupling
      seq_timemgr_alarm_gearun  = 'seq_timemgr_alarm_gearun  '  ! by juanxiong he for geatm/cam coupling
   ...
   integer(SHR_KIND_IN),private,parameter :: &
      ...
      seq_timemgr_nalarm_wrfrun  =14, &   ! by juanxiong he for wrf/cam coupling
      seq_timemgr_nalarm_gearun  =15   ! by juanxiong he for wrf/cam coupling
...
subroutine seq_timemgr_clockInit(SyncClock, nmlfile, restart, restart_file, mpicom, &
      ...
      Eclock_wrf, Eclock_gea )  ! by juanxiong he for wrf/cam coupling
...
! !INPUT/OUTPUT PARAMETERS:
   ...
   type(ESMF_clock),target,intent(IN)    :: EClock_wrf   ! by juanxiong he for wrf/cam coupling
   type(ESMF_clock),target,intent(IN)    :: EClock_gea   ! by juanxiong he for wrf/cam coupling
...
!EOP
    ...
    integer(SHR_KIND_IN)    :: wrf_cpl_dt            ! juanxiong he for wrf/ccsm4 coupling interval    
    integer(SHR_KIND_IN)    :: wrf_cpl_offset        ! juanxiong he wrf/ccsm4 coupling interval
    integer(SHR_KIND_IN)    :: gea_cpl_dt            ! juanxiong he for wrf/ccsm4 coupling interval    
    integer(SHR_KIND_IN)    :: gea_cpl_offset        ! juanxiong he wrf/ccsm4 coupling interval
    ...
    namelist /seq_timemgr_inparm/  calendar, curr_ymd, curr_tod,  &
         ...
         atm_cpl_dt, ocn_cpl_dt, ice_cpl_dt, lnd_cpl_dt, wrf_cpl_dt, &  ! by juanxiong he for wrf/cam coupling
         atm_cpl_offset, lnd_cpl_offset, ocn_cpl_offset, &
         ice_cpl_offset, glc_cpl_dt, glc_cpl_offset, wrf_cpl_offset, & ! by juanxiong he for wrf/cam coupling
         gea_cpl_dt, gea_cpl_offset, end_restart ! by juanxiong he for wrf/cam coupling
!-------------------------------------------------------------------------------
! Notes:
!-------------------------------------------------------------------------------
    ...
    SyncClock%ECP(seq_timemgr_nclock_wrf)%EClock => EClock_wrf ! by juanxiong he for wrf/cam coupling
    SyncClock%ECP(seq_timemgr_nclock_gea)%EClock => EClock_gea ! by juanxiong he for wrf/cam coupling
    ...
    if (iam == 0) then
       ...
       ocn_cpl_offset   = 0
       glc_cpl_offset   = 0
       wrf_cpl_dt       = 0  ! by juanxiong he for wrf/cam coupling
       wrf_cpl_offset   = 0  ! by juanxiong he for wrf/cam coupling  
       gea_cpl_dt       = 0  ! by juanxiong he for wrf/cam coupling
       gea_cpl_offset   = 0  ! by juanxiong he for wrf/cam coupling  
       end_restart      = .true.
    ...
    if (iam == 0) then
       ...
       if (wrf_cpl_dt == 0) wrf_cpl_dt = atm_cpl_dt ! by juanxiong he for wrf/cam coupling
       if (gea_cpl_dt == 0) gea_cpl_dt = atm_cpl_dt ! by juanxiong he for wrf/cam coupling
       ...
       write(logunit,F0I) trim(subname),' wrf_cpl_dt     = ',wrf_cpl_dt ! by juanxiong he for wrf/cam coupling
       write(logunit,F0I) trim(subname),' wrf_cpl_offset = ',wrf_cpl_offset ! by juanxiong he for wrf/cam coupling
       write(logunit,F0I) trim(subname),' gea_cpl_dt     = ',gea_cpl_dt ! by juanxiong he for wrf/cam coupling
       write(logunit,F0I) trim(subname),' gea_cpl_offset = ',gea_cpl_offset ! by juanxiong he for wrf/cam coupling
       write(logunit,F0A) ' '
       ...
       ! --- Coupling intervals ------------------------------------------------
       if ( wrf_cpl_dt <= 0 .or. & ! by juanxiong he for wrf/cam coupling
            gea_cpl_dt <= 0 .or. & ! by juanxiong he for wrf/cam coupling
            atm_cpl_dt <= 0 .or. &
          ...
          write(logunit,*) trim(subname),' ERROR: aliog _cpl_dt = ', &
             wrf_cpl_dt, gea_cpl_dt, atm_cpl_dt, lnd_cpl_dt, ice_cpl_dt, ocn_cpl_dt, glc_cpl_dt ! by juanxiong he for wrf/cam coupling
          call shr_sys_abort( subname//': ERROR coupling intervals invalid' )
       end if

       ! --- Coupling offsets --------------------------------------------------
       if ( abs(wrf_cpl_offset) > wrf_cpl_dt .or. & ! by juanxiong he for wrf/cam coupling
            abs(gea_cpl_offset) > gea_cpl_dt .or. & ! by juanxiong he for wrf/cam coupling
          ...
          write(logunit,*) trim(subname),' ERROR: aliog _cpl_offset = ', &
             wrf_cpl_offset, gea_cpl_offset, atm_cpl_offset, lnd_cpl_offset, ice_cpl_offset, ocn_cpl_offset, &  ! by juanxiong he for wrf/cam coupling
             glc_cpl_offset
          call shr_sys_abort( subname//': ERROR coupling intervals invalid' )
       end if
    ...
    call shr_mpi_bcast( wrf_cpl_dt,     mpicom ) ! by juanxiong he for wrf/cam coupling
    call shr_mpi_bcast( wrf_cpl_offset, mpicom ) ! by juanxiong he for wrf/cam coupling
    call shr_mpi_bcast( gea_cpl_dt,     mpicom ) ! by juanxiong he for wrf/cam coupling
    call shr_mpi_bcast( gea_cpl_offset, mpicom ) ! by juanxiong he for wrf/cam coupling
    ...
    dtime(seq_timemgr_nclock_wrf   ) = wrf_cpl_dt ! by juanxiong he for wrf/cam coupling
    dtime(seq_timemgr_nclock_gea   ) = gea_cpl_dt ! by juanxiong he for wrf/cam coupling
    ...
    offset(seq_timemgr_nclock_wrf) = wrf_cpl_offset ! by juanxiong he for wrf/cam coupling
    offset(seq_timemgr_nclock_gea) = gea_cpl_offset ! by juanxiong he for wrf/cam coupling
    ...
    ! added by juanxiong he for gea/ccsm4 coupling          
    call ESMF_TimeIntervalSet( TimeStep, s=offset(seq_timemgr_nclock_gea), rc=rc)
    OffsetTime = CurrTime + TimeStep
    call seq_timemgr_alarmInit(SyncClock%ECP(seq_timemgr_nclock_drv)%EClock, &
       EAlarm  = SyncClock%EAlarm(seq_timemgr_nclock_drv,seq_timemgr_nalarm_gearun),  &
       option  = seq_timemgr_optNSeconds,       &
       opt_n   = dtime(seq_timemgr_nclock_gea), &
       RefTime = OffsetTime,                    &
       alarmname = trim(seq_timemgr_alarm_gearun))

    ! added by juanxiong he for wrf/ccsm4 coupling          
    call ESMF_TimeIntervalSet( TimeStep, s=offset(seq_timemgr_nclock_wrf), rc=rc )
    OffsetTime = CurrTime + TimeStep
    call seq_timemgr_alarmInit(SyncClock%ECP(seq_timemgr_nclock_drv)%EClock, &
       EAlarm  = SyncClock%EAlarm(seq_timemgr_nclock_drv,seq_timemgr_nalarm_wrfrun),  &
       option  = seq_timemgr_optNSeconds,       &
       opt_n   = dtime(seq_timemgr_nclock_wrf), &
       RefTime = OffsetTime,                    &
       alarmname = trim(seq_timemgr_alarm_wrfrun))
...
!-------------------------------------------------------------------------------
   ...
   ! juanxiong he for gea/ccsm4 coupling           
   if (ESMF_AlarmIsRinging(SyncClock%EAlarm(seq_timemgr_nclock_drv,seq_timemgr_nalarm_gearun))) then
      call ESMF_ClockAdvance(SyncClock%ECP(seq_timemgr_nclock_gea)%EClock, rc=rc )
      call seq_timemgr_ESMFCodeCheck(rc, msg=subname//"Error from gea ESMF_ClockAdvance")
   endif

   ! juanxiong he for wrf/ccsm4 coupling
   if (ESMF_AlarmIsRinging(SyncClock%EAlarm(seq_timemgr_nclock_drv,seq_timemgr_nalarm_wrfrun))) then
      call ESMF_ClockAdvance(SyncClock%ECP(seq_timemgr_nclock_wrf)%EClock, rc=rc )
      call seq_timemgr_ESMFCodeCheck(rc, msg=subname//"Error from wrf ESMF_ClockAdvance")
   endif
