module seq_flds_mod
   ...
   integer, parameter :: CX = 80000           ! use local extra-long char  ! incerease up to 8000 for 3 dimension variables by juanxiong he
...
!-----------------------------------------------------------------------------------------------------------
  ...
  integer :: num_tracers
  parameter (num_cam_levs = 30,num_soil_layers = 15, num_tracers = 3)
  character*2, dimension(1:num_cam_levs ) :: clev
  character*2, dimension(1:num_soil_layers) :: slayer
  character*2, dimension(1:num_tracers) :: ctracer

   character(CX) :: seq_flds_ge2x_states
   character(CX) :: seq_flds_ge2x_fluxes
   character(CX) :: seq_flds_x2ge_states
   character(CX) :: seq_flds_x2ge_fluxes
   character(CX) :: seq_flds_ca2x_states
   character(CX) :: seq_flds_ca2x_fluxes
   character(CX) :: seq_flds_x2ca_states
   character(CX) :: seq_flds_x2ca_fluxes
   character(CX) :: seq_flds_x2ca_fields
   character(CX) :: seq_flds_ca2x_fields
   character(CX) :: seq_flds_x2ge_fields
   character(CX) :: seq_flds_ge2x_fields
!-----------------------------------------------------------------------------------------------------------
!    for geatm-cam coupling
!    Juanxiong He, 06/10/2013
!-----------------------------------------------------------------------------------------------------------  
...
!-----------------------------------------------------------------------------------------------------------
   character(CX) :: seq_flds_w2x_states
   character(CX) :: seq_flds_w2x_fluxes
   character(CX) :: seq_flds_x2w_states
   character(CX) :: seq_flds_x2w_fluxes
   character(CX) :: seq_flds_c2x_states
   character(CX) :: seq_flds_c2x_fluxes
   character(CX) :: seq_flds_x2c_states
   character(CX) :: seq_flds_x2c_fluxes
   character(CX) :: seq_flds_x2c_fields
   character(CX) :: seq_flds_c2x_fields
   character(CX) :: seq_flds_x2w_fields
   character(CX) :: seq_flds_w2x_fields
!-----------------------------------------------------------------------------------------------------------
!    for wrf-cam coupling
!    Juanxiong He, 05/10/2010
!-----------------------------------------------------------------------------------------------------------  
   ...
   character(32) :: seq_flds_geatmname='geatm'  ! by juanxiong he for geatm-cam coupling
   character(32) :: seq_flds_gcamname='gcam'  ! by juanxiong he for geatm-cam coupling 
   character(32) :: seq_flds_wrfname='wrf'  ! by juanxiong he for wrf-cam coupling
   character(32) :: seq_flds_camname='cam'  ! by juanxiong he for wrf-cam coupling     
...
subroutine seq_flds_geatm_set()
   integer :: i, j, k

   do k = 1, num_cam_levs
   clev(k)= char(k/10+48)//char(k-k/10*10+48)
   end do
   do k = 1, num_soil_layers
   slayer(k) = char(k/10+48)//char(k-k/10*10+48)
   end do
   do k = 1, num_tracers
   ctracer(k)= char(k/10+48)//char(k-k/10*10+48)
   end do
   
   ! drv -> geatm 
   seq_flds_x2ge_states ='Sx_u3d01' 
   do k=2, num_cam_levs
   seq_flds_x2ge_states = trim(adjustl(seq_flds_x2ge_states))//':Sx_u3d'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_x2ge_states = trim(adjustl(seq_flds_x2ge_states))//':Sx_v3d'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_x2ge_states = trim(adjustl(seq_flds_x2ge_states))//':Sx_t3d'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_x2ge_states = trim(adjustl(seq_flds_x2ge_states))//':Sx_z3d'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_x2ge_states = trim(adjustl(seq_flds_x2ge_states))//':Sx_p3d'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_x2ge_states = trim(adjustl(seq_flds_x2ge_states))//':Sx_rh3d'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_x2ge_states = trim(adjustl(seq_flds_x2ge_states))//':Sx_qi3d'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_x2ge_states = trim(adjustl(seq_flds_x2ge_states))//':Sx_qv3d'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_x2ge_states = trim(adjustl(seq_flds_x2ge_states))//':Sx_qc3d'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_x2ge_states = trim(adjustl(seq_flds_x2ge_states))//':Sx_taucldv3d'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_x2ge_states = trim(adjustl(seq_flds_x2ge_states))//':Sx_taucldi3d'//clev(k)
   end do
   do k=1, num_soil_layers
   seq_flds_x2ge_states = trim(adjustl(seq_flds_x2ge_states))//':Sx_soilt'//slayer(k)
   end do
   do k=1, num_soil_layers
   seq_flds_x2ge_states = trim(adjustl(seq_flds_x2ge_states))//':Sx_soilm'//slayer(k)
   end do
   do k=1, num_soil_layers
   seq_flds_x2ge_states = trim(adjustl(seq_flds_x2ge_states))//':Sx_soildepth'//slayer(k)
   end do
   do k=1, num_soil_layers
   seq_flds_x2ge_states = trim(adjustl(seq_flds_x2ge_states))//':Sx_soilthick'//slayer(k)
   end do
   seq_flds_x2ge_states = trim(adjustl(seq_flds_x2ge_states))//':Sx_ps'//':Sx_phis'//':Sx_lat' &
                          //':Sx_lon'//':Sx_ts'//':Sx_sst'//':Sx_snowhland'//':Sx_t2' &
                          //':Sx_snowhice'//':Sx_seaice'//':Sx_ocnfrac'//':Sx_u10'//':Sx_q2' &
                          //':Sx_rh2'//':Sx_v10'//':Sx_ust'//':Sx_rmol'//':Sx_pblh'//':Sx_raincv' &
                          //':Sx_rainncv'//':Sx_swdown'//':Sx_clflo'//':Sx_clfmi'//':Sx_clfhi' 
   seq_flds_x2ge_fluxes =''

   ! geatm -> drv
   seq_flds_ge2x_states = 'Fgexge_tracer0101'
   do k=2, num_cam_levs
   seq_flds_ge2x_states = trim(adjustl(seq_flds_ge2x_states))//':Fgexge_tracer'//clev(k)//'01'
   end do
   do i=2, num_tracers
   do k=1, num_cam_levs
   seq_flds_ge2x_states = trim(adjustl(seq_flds_ge2x_states))//':Fgexge_tracer'//clev(k)//ctracer(i)
   end do
   end do
   seq_flds_ge2x_fluxes = ''

   ! cam -> drv
   seq_flds_ca2x_states = 'Sca_u3d01'
   do k=2, num_cam_levs
   seq_flds_ca2x_states = trim(adjustl(seq_flds_ca2x_states))//':Sca_u3d'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_ca2x_states = trim(adjustl(seq_flds_ca2x_states))//':Sca_v3d'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_ca2x_states = trim(adjustl(seq_flds_ca2x_states))//':Sca_t3d'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_ca2x_states = trim(adjustl(seq_flds_ca2x_states))//':Sca_z3d'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_ca2x_states = trim(adjustl(seq_flds_ca2x_states))//':Sca_p3d'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_ca2x_states = trim(adjustl(seq_flds_ca2x_states))//':Sca_rh3d'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_ca2x_states = trim(adjustl(seq_flds_ca2x_states))//':Sca_qi3d'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_ca2x_states = trim(adjustl(seq_flds_ca2x_states))//':Sca_qv3d'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_ca2x_states = trim(adjustl(seq_flds_ca2x_states))//':Sca_qc3d'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_ca2x_states = trim(adjustl(seq_flds_ca2x_states))//':Sca_taucldv3d'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_ca2x_states = trim(adjustl(seq_flds_ca2x_states))//':Sca_taucldi3d'//clev(k)
   end do
   do k=1, num_soil_layers
   seq_flds_ca2x_states = trim(adjustl(seq_flds_ca2x_states))//':Sca_soilt'//slayer(k)
   end do
   do k=1, num_soil_layers
   seq_flds_ca2x_states = trim(adjustl(seq_flds_ca2x_states))//':Sca_soilm'//slayer(k)
   end do
   do k=1, num_soil_layers
   seq_flds_ca2x_states = trim(adjustl(seq_flds_ca2x_states))//':Sca_soildepth'//slayer(k)
   end do
   do k=1, num_soil_layers
   seq_flds_ca2x_states = trim(adjustl(seq_flds_ca2x_states))//':Sca_soilthick'//slayer(k)
   end do
   seq_flds_ca2x_states = trim(adjustl(seq_flds_ca2x_states))//':Sca_ps'//':Sca_phis'//':Sca_lat'&
                          //':Sca_lon'//':Sca_ts'//':Sca_sst'//':Sca_snowhland' &
                          //':Sca_snowhice'//':Sca_seaice'//':Sca_ocnfrac'//':Sca_t2' &
                          //':Sca_q2'//':Sca_rh2'//':Sca_u10' &
                          //':Sca_v10'//':Sca_ust'//':Sca_rmol'//':Sca_pblh'//':Sca_raincv' &
                          //':Sca_rainncv'//':Sca_swdown'//':Sca_clflo'//':Sca_clfmi'//':Sca_clfhi' 

   seq_flds_ca2x_fluxes =''

   ! drv -> cam
   seq_flds_x2ca_states = 'Fcaxx_tracer0101'
   do k=2, num_cam_levs
   seq_flds_x2ca_states = trim(adjustl(seq_flds_x2ca_states))//':Fcaxx_tracer'//clev(k)//'01'
   end do
   do i=2, num_tracers
   do k=1, num_cam_levs
   seq_flds_x2ca_states = trim(adjustl(seq_flds_x2ca_states))//':Fcaxx_tracer'//clev(k)//ctracer(i)
   end do
   end do
   seq_flds_x2ca_fluxes =''

end subroutine seq_flds_geatm_set

subroutine seq_flds_wrf_set()
   integer :: i, j, k
...
!-----------------------------------------------------------------------------------------------------------
   ...
   ! States
   seq_flds_w2x_states = 'Sw_u3d01'
   do k=2, num_cam_levs
   seq_flds_w2x_states = trim(adjustl(seq_flds_w2x_states))//':Sw_u3d'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_w2x_states = trim(adjustl(seq_flds_w2x_states))//':Sw_v3d'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_w2x_states = trim(adjustl(seq_flds_w2x_states))//':Sw_t3d'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_w2x_states = trim(adjustl(seq_flds_w2x_states))//':Sw_q3d'//clev(k)
   end do
   seq_flds_w2x_states = trim(adjustl(seq_flds_w2x_states))//':Sw_ps'

   ! Fluxes
   seq_flds_w2x_fluxes = 'Fwxw_dudt01'
   do k=2, num_cam_levs
   seq_flds_w2x_fluxes = trim(adjustl(seq_flds_w2x_fluxes))//':Fwxw_dudt'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_w2x_fluxes = trim(adjustl(seq_flds_w2x_fluxes))//':Fwxw_dvdt'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_w2x_fluxes = trim(adjustl(seq_flds_w2x_fluxes))//':Fwxw_dtdt'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_w2x_fluxes = trim(adjustl(seq_flds_w2x_fluxes))//':Fwxw_dqdt'//clev(k)
   end do
   ...
   ! State
   seq_flds_x2w_states = 'Sx_z3d01'
   do k=2, num_cam_levs
   seq_flds_x2w_states = trim(adjustl(seq_flds_x2w_states))//':Sx_z3d'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_x2w_states = trim(adjustl(seq_flds_x2w_states))//':Sx_u3d'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_x2w_states = trim(adjustl(seq_flds_x2w_states))//':Sx_v3d'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_x2w_states = trim(adjustl(seq_flds_x2w_states))//':Sx_t3d'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_x2w_states = trim(adjustl(seq_flds_x2w_states))//':Sx_w3d'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_x2w_states = trim(adjustl(seq_flds_x2w_states))//':Sx_q3d'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_x2w_states = trim(adjustl(seq_flds_x2w_states))//':Sx_p3d'//clev(k)
   end do
   do k=1, num_soil_layers
   seq_flds_x2w_states = trim(adjustl(seq_flds_x2w_states))//':Sx_soilt'//slayer(k)
   end do
   do k=1, num_soil_layers
   seq_flds_x2w_states = trim(adjustl(seq_flds_x2w_states))//':Sx_soilm'//slayer(k)
   end do
   do k=1, num_soil_layers
   seq_flds_x2w_states = trim(adjustl(seq_flds_x2w_states))//':Sx_soildepth'//slayer(k)
   end do
   do k=1, num_soil_layers
   seq_flds_x2w_states = trim(adjustl(seq_flds_x2w_states))//':Sx_soilthick'//slayer(k)
   end do
   seq_flds_x2w_states = trim(adjustl(seq_flds_x2w_states))//':Sx_ps'  & ! atm surface pressure
      //':Sx_phis'&   ! atm surface geopotential height
      //':Sx_lat'&   ! atm latitude
      //':Sx_lon'&   ! atm longitude
      //':Sx_ts' & !surface temperature           
      //':Sx_sst' &! sst   
      //':Sx_snowhland' & ! snow height over land  
      //':Sx_snowhice' &!snow height over ice    
      //':Sx_seaice' & ! seaice  
      //':Sx_ocnfrac' !ocn fraction      

   seq_flds_x2w_fluxes = 'Fxw_ttend01'
   do k=2, num_cam_levs
   seq_flds_x2w_fluxes = trim(adjustl(seq_flds_x2w_fluxes))//':Fxw_ttend'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_x2w_fluxes = trim(adjustl(seq_flds_x2w_fluxes))//':Fxw_qtend'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_x2w_fluxes = trim(adjustl(seq_flds_x2w_fluxes))//':Fxw_vtend'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_x2w_fluxes = trim(adjustl(seq_flds_x2w_fluxes))//':Fxw_utend'//clev(k)
   end do
   ...
   ! State
   seq_flds_c2x_states = 'Sc_z3d01'
   do k=2, num_cam_levs
   seq_flds_c2x_states = trim(adjustl(seq_flds_c2x_states))//':Sc_z3d'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_c2x_states = trim(adjustl(seq_flds_c2x_states))//':Sc_u3d'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_c2x_states = trim(adjustl(seq_flds_c2x_states))//':Sc_v3d'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_c2x_states = trim(adjustl(seq_flds_c2x_states))//':Sc_t3d'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_c2x_states = trim(adjustl(seq_flds_c2x_states))//':Sc_w3d'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_c2x_states = trim(adjustl(seq_flds_c2x_states))//':Sc_q3d'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_c2x_states = trim(adjustl(seq_flds_c2x_states))//':Sc_p3d'//clev(k)
   end do
   do k=1, num_soil_layers
   seq_flds_c2x_states = trim(adjustl(seq_flds_c2x_states))//':Sc_soilt'//slayer(k)
   end do
   do k=1, num_soil_layers
   seq_flds_c2x_states = trim(adjustl(seq_flds_c2x_states))//':Sc_soilm'//slayer(k)
   end do
   do k=1, num_soil_layers
   seq_flds_c2x_states = trim(adjustl(seq_flds_c2x_states))//':Sc_soildepth'//slayer(k)
   end do
   do k=1, num_soil_layers
   seq_flds_c2x_states = trim(adjustl(seq_flds_c2x_states))//':Sc_soilthick'//slayer(k)
   end do
   seq_flds_c2x_states = trim(adjustl(seq_flds_c2x_states))//':Sc_ps' & ! atm surface pressure                  
      //':Sc_phis'&  ! atm surface geopotential height
      //':Sc_lat'&  ! atm latitude
      //':Sc_lon' &  ! atm longitude
      //':Sc_ts' & !surface temperature           
      //':Sc_sst' &! sst   
      //':Sc_snowhland' & ! snow height over land  
      //':Sc_snowhice' &!snow height over ice    
      //':Sc_seaice' & ! seaice  
      //':Sc_ocnfrac' !ocn fraction   

   seq_flds_c2x_fluxes = 'Fcxc_ttend01'
   do k=2, num_cam_levs
   seq_flds_c2x_fluxes = trim(adjustl(seq_flds_c2x_fluxes))//':Fcxc_ttend'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_c2x_fluxes = trim(adjustl(seq_flds_c2x_fluxes))//':Fcxc_qtend'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_c2x_fluxes = trim(adjustl(seq_flds_c2x_fluxes))//':Fcxc_vtend'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_c2x_fluxes = trim(adjustl(seq_flds_c2x_fluxes))//':Fcxc_utend'//clev(k)
   end do
   ...
   seq_flds_x2c_states = 'Sx_u3d01'
   do k=2, num_cam_levs
   seq_flds_x2c_states = trim(adjustl(seq_flds_x2c_states))//':Sx_u3d'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_x2c_states = trim(adjustl(seq_flds_x2c_states))//':Sx_v3d'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_x2c_states = trim(adjustl(seq_flds_x2c_states))//':Sx_t3d'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_x2c_states = trim(adjustl(seq_flds_x2c_states))//':Sx_q3d'//clev(k)
   end do
   seq_flds_x2c_states = trim(adjustl(seq_flds_x2c_states))//':Sx_ps'

   ! Fluxes
   seq_flds_x2c_fluxes = 'Fcxx_dudt01'
   do k=2, num_cam_levs
   seq_flds_x2c_fluxes = trim(adjustl(seq_flds_x2c_fluxes))//':Fcxx_dudt'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_x2c_fluxes = trim(adjustl(seq_flds_x2c_fluxes))//':Fcxx_dvdt'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_x2c_fluxes = trim(adjustl(seq_flds_x2c_fluxes))//':Fcxx_dtdt'//clev(k)
   end do
   do k=1, num_cam_levs
   seq_flds_x2c_fluxes = trim(adjustl(seq_flds_x2c_fluxes))//':Fcxx_dqdt'//clev(k)
   end do
...
end subroutine seq_flds_wrf_set
...
!-----------------------------------------------------------------------------------------------------------
   ...
   call seq_flds_geatm_set()
   call seq_flds_wrf_set()
!-----------------------------------------------------------------------------------------------------------
!    for wrf-geatm-cam coupling
!    Juanxiong He, 05/10/2010
!-----------------------------------------------------------------------------------------------------------
   ...
   seq_flds_x2a_states = &
      ...
      //':Sx_u10'          ! 10m wind                        DEF
!-----------------------------------------------------------------------------------------------------------
!   soil depth/thickness, temperature/moisture for wrf-cam coupling    Juanxiong He
!-----------------------------------------------------------------------------------------------------------      
   do k=1, num_soil_layers
   seq_flds_x2a_states = trim(adjustl(seq_flds_x2a_states))//':Sx_soilt'//slayer(k)
   end do
   do k=1, num_soil_layers
   seq_flds_x2a_states = trim(adjustl(seq_flds_x2a_states))//':Sx_soilm'//slayer(k)
   end do
   do k=1, num_soil_layers
   seq_flds_x2a_states = trim(adjustl(seq_flds_x2a_states))//':Sx_soildepth'//slayer(k)
   end do
   do k=1, num_soil_layers
   seq_flds_x2a_states = trim(adjustl(seq_flds_x2a_states))//':Sx_soilthick'//slayer(k)
   end do
!-----------------------------------------------------------------------------------------------------------
!   soil depth/thickness, temperature/moisture for wrf-cam coupling    Juanxiong He
!-----------------------------------------------------------------------------------------------------------  
...
!-----------------------------------------------------------------------------------------------------------      
   do k=1, num_soil_layers
   seq_flds_l2x_states = trim(adjustl(seq_flds_l2x_states))//':Sl_soilt'//slayer(k)
   end do
   do k=1, num_soil_layers
   seq_flds_l2x_states = trim(adjustl(seq_flds_l2x_states))//':Sl_soilm'//slayer(k)
   end do
   do k=1, num_soil_layers
   seq_flds_l2x_states = trim(adjustl(seq_flds_l2x_states))//':Sl_soildepth'//slayer(k)
   end do
   do k=1, num_soil_layers
   seq_flds_l2x_states = trim(adjustl(seq_flds_l2x_states))//':Sl_soilthick'//slayer(k)
   end do
!-----------------------------------------------------------------------------------------------------------
!   soil depth/thickness, temperature/moisture for wrf-cam coupling    Juanxiong He
!-----------------------------------------------------------------------------------------------------------   
...
!    Juanxiong He, 05/10/2010
!-----------------------------------------------------------------------------------------------------------       
    if (len_trim(seq_flds_w2x_states ) .ge. len(seq_flds_w2x_states )) &
        call shr_sys_abort('maximum length of string has been exceeded for: seq_flds_w2x_states ')
    if (len_trim(seq_flds_x2w_states ) .ge. len(seq_flds_x2w_states )) &
        call shr_sys_abort('maximum length of string has been exceeded for: seq_flds_x2w_states ')
    if (len_trim(seq_flds_w2x_fluxes ) .ge. len(seq_flds_w2x_fluxes )) &
        call shr_sys_abort('maximum length of string has been exceeded for: seq_flds_w2x_fluxes ')
    if (len_trim(seq_flds_x2w_fluxes ) .ge. len(seq_flds_x2w_fluxes )) &
        call shr_sys_abort('maximum length of string has been exceeded for: seq_flds_x2w_fluxes ')

    if (len_trim(seq_flds_c2x_states ) .ge. len(seq_flds_c2x_states )) &
        call shr_sys_abort('maximum length of string has been exceeded for: seq_flds_c2x_states ')
    if (len_trim(seq_flds_x2c_states ) .ge. len(seq_flds_x2c_states )) &
        call shr_sys_abort('maximum length of string has been exceeded for: seq_flds_x2c_states ')
    if (len_trim(seq_flds_c2x_fluxes ) .ge. len(seq_flds_c2x_fluxes )) &
        call shr_sys_abort('maximum length of string has been exceeded for: seq_flds_c2x_fluxes ')
    if (len_trim(seq_flds_x2c_fluxes ) .ge. len(seq_flds_x2c_fluxes )) &
        call shr_sys_abort('maximum length of string has been exceeded for: seq_flds_c2x_fluxes ')

    if ((len_trim(seq_flds_w2x_states) + len_trim(seq_flds_w2x_fluxes)) .ge. len(seq_flds_w2x_fields)) &
        call shr_sys_abort('maximum length of string has been exceeded for: seq_flds_w2x_fields')
    if ((len_trim(seq_flds_x2w_states) + len_trim(seq_flds_x2w_fluxes)) .ge. len(seq_flds_x2w_fields)) &
        call shr_sys_abort('maximum length of string has been exceeded for: seq_flds_x2w_fields')
    if ((len_trim(seq_flds_c2x_states) + len_trim(seq_flds_c2x_fluxes)) .ge. len(seq_flds_c2x_fields)) &
        call shr_sys_abort('maximum length of string has been exceeded for: seq_flds_c2x_fields')
    if ((len_trim(seq_flds_x2c_states) + len_trim(seq_flds_x2c_fluxes)) .ge. len(seq_flds_x2c_fields)) &
        call shr_sys_abort('maximum length of string has been exceeded for: seq_flds_x2c_fields')
!-----------------------------------------------------------------------------------------------------------
!    for wrf-cam coupling
!    Juanxiong He, 05/10/2010
!-----------------------------------------------------------------------------------------------------------   
...
!    Juanxiong He, 05/10/2010
!----------------------------------------------------------------------------------------------------------- 
    call seq_flds_catFields(seq_flds_c2x_fields,seq_flds_c2x_states,seq_flds_c2x_fluxes )
    call seq_flds_catFields(seq_flds_x2c_fields,seq_flds_x2c_states,seq_flds_x2c_fluxes )
    call seq_flds_catFields(seq_flds_x2w_fields,seq_flds_x2w_states,seq_flds_x2w_fluxes )
    call seq_flds_catFields(seq_flds_w2x_fields,seq_flds_w2x_states,seq_flds_w2x_fluxes )
!-----------------------------------------------------------------------------------------------------------
!    for wrf-cam coupling
!    Juanxiong He, 05/10/2010
!-----------------------------------------------------------------------------------------------------------   
...
!-----------------------------------------------------------------------------------------------------------
!    for wrf-geatm coupling
!    Juanxiong He, 06/10/2013
!----------------------------------------------------------------------------------------------------------- 
    call seq_flds_catFields(seq_flds_ca2x_fields,seq_flds_ca2x_states,seq_flds_ca2x_fluxes )
    call seq_flds_catFields(seq_flds_x2ca_fields,seq_flds_x2ca_states,seq_flds_x2ca_fluxes )
    call seq_flds_catFields(seq_flds_x2ge_fields,seq_flds_x2ge_states,seq_flds_x2ge_fluxes )
    call seq_flds_catFields(seq_flds_ge2x_fields,seq_flds_ge2x_states,seq_flds_ge2x_fluxes )
!-----------------------------------------------------------------------------------------------------------
!    for wrf-geatm coupling
!    Juanxiong He, 06/10/2013
!----------------------------------------------------------------------------------------------------------- 
